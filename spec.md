...
- **Ізоляція сиріт (`ensureOrphan`, `resolvableOrphan`, `ensureBeerRow`).** Сироти з порожнім `normalized_name`
  вважаються сумісними лише тоді, коли їхні `styleNameIdentity` збігаються. Картка крамниці чи кран одного стилю
  (`Stout`) ніколи не бере рядок чи сироту іншого стилю (`Lager`), навіть якщо обидва мають `normalized_name === ''`.
