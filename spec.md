...
- **Три детерміновані гейти вердикту** в `planTriageActions` (чиста функція, без
  ...
  2. Вердикт приліплюється до відкритої issue, лише якщо рядок задовольняє її scope.
     ...
     `new_issue_key`) знімається, а `review_note` отримує префікс `off-scope <ціль>: <причина>`
     (перший term, якому рядок суперечить, напр. `candidates_count = 0`; якщо в issue взагалі
     немає блоку scope — літерально `no scope block`, а не «outside the cohort», бо рядок не
     провалив належність до когорти — в issue просто нема scope). Рахується окремо від
     `guardHits.scope_violation` (той лічить спрацювання гейта) як `quietOffScope`
     (`planTriageActions`) → `outcome.offScope`, і публікується в дайджесті як
     `N без власника (поза scope)`.
