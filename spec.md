...
`24°`, мітки `alc/abv/ibu`); компактні ідентифікатори без пробілів у дужках
зберігає.
...
текстово не збігаються з каталогом. Якщо очистка спорожнила запит — фолбек на очищену
назву/броварню, ніколи не на сиру (щоб не реін'єктити дужки).
...
ідентифікатори без пробілів у дужках (наприклад, `(TAP04)`) зберігаються.
...
- **Ontap non-beer gate.** `refreshOntap` ПОВИНЕН відкидати очевидні не-пивні
  ...
  створення snapshot/tap рядків, matcher/upsert orphan і enrich. Сигнали gate — тільки `taps.style` і
...
- **Стратегія ротації exit-IP (#222).** `p.webshare.io` не ротує в межах однієї
  ...
  як захват акаунту). Обидва клієнти виконують **один ретрай на свіжому IP** до
