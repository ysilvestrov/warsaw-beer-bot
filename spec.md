...
- **Ціль, яку гейт відмовить напевно, моделі не пропонується (#509).** Issue без
  scope-блоку або з когортою без `where` відмовляє КОЖНОМУ рядку поза перерахованою
  когортою за побудовою — такі issue фільтруються з відкритого списку, який бачить
  модель у промпті. Фільтр стосується лише промпту: `planTriageActions` і звірка
