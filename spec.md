...
(ймовірно випите, без певності); у розширенні дає бейдж `❓` з глобальним рейтингом і
кліком на Untappd. `user_rating` — особиста оцінка з `checkins` (або `null`); лише для
...
- **Per-site адаптери** (`src/sites/`): кожен має стабільний `id` (= ім'я
  ...
  домен `hoptimaal.com`), `flasker` (Flasker WooCommerce SSR — `li.product`/`h2.woocommerce-loop-product__title` (archive), `tr[data-title]` (Barn2 product table), `li.wc-block-grid__product` (block grid); brewery/name з першого слова тайтлу (volume-gate: пиво завжди містить об'єм в ml/л/l, non-beer без об'єму відкидається; ABV із `%` у тайтлі),
  домен `flasker.com.ua`). `registry.pickAdapter(url)`.
...
- **Бейджі.** Питі беври (exact-матч) — `✅` + особиста оцінка. Каталожні беври, які
  ...
  глобальний рейтинг («ймовірно випите, без певності»). Будь-який бейдж із `untappd_id`
  клікабельний: відкриває сторінку беври на Untappd
