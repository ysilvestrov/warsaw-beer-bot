...
| `rescued_lookup_count`/`rescued_lookup_at`/`rescued_rearm_count`/`rescued_probed_at` | INTEGER/TEXT/INTEGER/TEXT | nullable (міграція 36) | знімок стану рядка й час проби. Closeout і unlock приймають його тільки при збігу з поточними полями; новий реальний провал або зміна issue/класу інвалідують маркер |
...
- **Проба** — `npm run adjudicate -- --issue <n>` перевіряє канарку до й після, читає нематчені/неретайрені рядки issue та пише файл вердиктів у `/tmp`. Кожен має brewery/name і lookup count/time/rearm count; `rescued` додатково має matched bid і ABV входу (`null` допустимий). Провал канарки або відкритий circuit breaker не дає застосовного файлу. Старий `unrescued_at` дає `already_marked` без нового пошуку — це не дозвіл закрити issue; для нового реплею потрібен явний rearm.
- **Застосування** — `npm run adjudicate -- --apply <файл>` суворо розбирає файл (старий `rescued` без bid/ABV відхиляється), звіряє кожен позитивний і негативний вердикт із поточними issue, brewery/name, ABV для позитивного, lookup count/time і `rearm_count`, після чого в одній транзакції пише `rescued_*` або `unrescued_*`. Зрушений рядок отримує явну причину `skipped`; `--force` обходить лише чотиригодинне вікове вікно файлу, не ці звірки. Позитивний proof інвалідується новим реальним провалом, зміною issue/класу або розбіжністю поточного знімка; активне рішення #695 також забороняє unlock.
