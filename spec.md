...
- **Три детерміновані гейти вердикту** в `planTriageActions` (чиста функція, без
  ...
  2. Вердикт приліплюється до відкритої issue, лише якщо рядок задовольняє її scope.
     ...
     обидва модельні за походженням без обмеження символів (`new_issue_key`; значення
     `contains`-терма, яке потрапляє в `<причина>` через `describeTerm`) — санітизуються
     перед склеюванням: `: ` у цілі й `|` в обох замінюються (на `; ` і `/` відповідно),
     щоб `groupOwnerless` міг розпарсити ціль і причину однозначно, а не вгадувати межу
     регексом; людина в інбоксі бачить той самий сенс, лише без цих символів буквально.
     Усі три модельні поля (ціль, причина й оригінальна нотатка моделі) перед цим
     проходять ще один крок: будь-яка послідовність пробільних символів (переноси рядка,
     таби, кілька пробілів підряд) стягується в один пробіл, тому нотатка з переносом
     рядка в інбоксі читається як один рядок, а не так, як її дослівно написала модель.
     Складений рядок обрізається до 500 символів власним `.slice(0, 500)` у
     `refuseRoute` —
     `VerdictSchema` тут ні до чого, вона обмежує лише вхідну нотатку моделі ще до
     склеювання. Оригінальна репліка моделі («shop brand in brewery field» тощо) навмисно
