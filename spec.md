...
- **Popup — ієрархія подання:** порядок у DOM = порядок за важливістю —
  ...
  Первинна дія йде **першою**, тож перший `Tab` потрапляє на неї, а не на
  деструктивну. Стан `disabled` має власні токени (заливка + приглушений текст),
  а не `opacity`. Popup підхоплює тему через `color-scheme: light dark`; акцент
