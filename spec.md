...
- **Popup — ієрархія подання:** порядок у DOM = порядок за важливістю —
  «Sync my check-ins» (primary, залитий акцентом) → «Refresh this page»
  (secondary, контур) → згорнутий список «Supported shops» → «Clear all cache»
  (ghost, під лінією-роздільником). Список групує магазини за країною, з якої
  ...
  неї, а не на деструктивну. **Залишковий випадок:** якщо синхронізація вже йде
  (`syncCheckins` disabled) **і** вкладка не з підтримуваного магазину
  (`refresh` disabled), обидві кнопки випадають із tab-order, і перший `Tab`
  знову потрапляє на «Clear all cache» — це знімається лише підтвердженням
  деструктивної дії (#517). Стан `disabled` має власні токени (заливка +
