...
| `unlocked_at` | TEXT | nullable (міграція 25) | час, коли рядок отримав **безкоштовний ретрай** після виходу його issue з набору відкритих і перевірки чинного позитивного proof (#697). Не «розблоковано назавжди», а «ставка в польоті»: вердикт при цьому **зберігається**, бо ми його не спростували, а перевіряємо. Скидається в `NULL`, щойно ставка закривається — успіхом (рядок зникає) або новим провалом (вердикт знімається) |
...
| `unrescued_at` | TEXT | nullable (міграція 27) | негативний реплей (#558): цей фікс не рятує рядок. Пулів не змінює; не дозволяє закрити issue і не дає `unlock-fixed-orphans` ані зняти замок, ані зробити rearm (#697). Знімається разом із `unrescued_issue` явним `rearmLookup`, щоб після нового доказу рядок можна було перепробувати |
...
| `rescued_issue`/`rescued_at`/`rescued_bid` | INTEGER/TEXT/INTEGER | nullable (міграція 36) | застосований позитивний реплей для конкретного issue, час застосування і знайдений bid. Без бекфілу; сам факт закриття issue чи старий `retired_at` не створює доказу |
| `rescued_brewery`/`rescued_name`/`rescued_abv` | TEXT/TEXT/REAL | nullable (міграція 36) | точний вхід пошуку на момент позитивного реплею; `rescued_abv = NULL` є доказовою відсутністю ABV, не пропущеним полем |
| `rescued_lookup_count`/`rescued_lookup_at`/`rescued_rearm_count`/`rescued_probed_at` | INTEGER/TEXT/INTEGER/TEXT | nullable (міграція 36) | знімок lookup/rearm і час проби. Closeout і unlock приймають його тільки при збігу з поточними полями |
| `real_failure_count`/`rescued_real_failure_count` | INTEGER/INTEGER | 0 за замовчуванням / nullable (міграція 37) | покоління лише реальних результатів `not_found` і його знімок при позитивному реплеї. Мережеве `blocked` не збільшує покоління. Apply звіряє ці поля, щоб старий файл не поставив доказ після нового реального провалу; після застосування новий реальний провал окремо очищує маркер |
...
- джоба `unlock-fixed-orphans` (щогодинний тік, раз на варшавську добу через `job_state`) звіряє `issue_number` заблокованих рядків із набором відкритих issue за лейблом `orphan-triage`. Вихід issue із набору — тільки тригер перевірки. **Такт 1** (`rearmLookup` + `unlocked_at`) отримує лише рядок із чинним позитивним `rescued_*` доказом для свого issue та незмінних входу/lookup/rearm-стану. `unrescued_at`, відсутній або застарілий proof залишають рядок під замком без зміни backoff; активне рішення #695 теж ніколи не розмикається. Утримані ID/issue зберігаються в `job_state` і показуються в щоденному статусі;
...
- GitHub сам не доводить, що фікс приїхав (`closedByPullRequestsReferences` порожній, `stateReason` відображає натиснуту кнопку). Тому закриття або ручне зняття мітки не є доказом і без чинного позитивного реплею не купує жодного лукапу;
...
**Адюдикація (#558, #576, #697).** Закриття issue лише запускає перевірку; дозвіл на повторну спробу дає застосований позитивний реплей конкретного рядка. Проба й застосування розведені, щоб жоден маркер не з'явився, поки канарка не пройшла **до і після всього прогону**. Проба не пише БД; apply звіряє живий стан транзакційно й не торкається мережі:

- **Проба** — `npm run adjudicate -- --issue <n>` перевіряє канарку до й після, читає нематчені/неретайрені рядки issue та пише файл вердиктів у `/tmp`. Кожен має brewery/name і lookup count/time/rearm count; `rescued` додатково має matched bid, ABV входу (`null` допустимий) та `real_failure_count`. Провал канарки або відкритий circuit breaker не дає застосовного файлу. Старий `unrescued_at` дає `already_marked` без нового пошуку — це не дозвіл закрити issue; для нового реплею потрібен явний rearm.
- **Застосування** — `npm run adjudicate -- --apply <файл>` суворо розбирає файл (старий `rescued` без bid/ABV/`real_failure_count` відхиляється), звіряє кожен позитивний і негативний вердикт із поточними issue, brewery/name, ABV для позитивного, lookup count/time і `rearm_count`, а позитивний — також із поточним `real_failure_count`; потім в одній транзакції пише `rescued_*` або `unrescued_*`. Зрушений рядок отримує явну причину `skipped`; `--force` обходить лише чотиригодинне вікове вікно файлу, не перевірки стану чи заборону часу проби в майбутньому. Позитивний proof інвалідується новим реальним провалом, зміною issue/класу або розбіжністю поточного знімка; активне рішення #695 також забороняє unlock.
...
**Закриття issue (#697).** `npm run close-orphan-issue -- --issue <n>` читає GitHub issue та всі живі рядки, що її називають, без ліміту; показує `rescued` з чинним proof, `inactive` з активним рішенням #695 для тієї самої issue й точного ключа або `blocked`. Історичні ремонти #696 показуються з `legacy_card_repairs`, але не виправдовують живого рядка. `retired_at`, `unrescued_at`, inconclusive, неузгоджений matched-рядок і відсутність proof — блокери. Лише явний `--close` повторює перевірку перед GitHub PATCH і звіряє стан після нього. GitHub UI може обійти команду, але не фонову перевірку proof. Не існує спільної транзакції SQLite/GitHub, тому нові рядки між preflight і PATCH можуть лишити issue закритою; постперевірка повідомляє про це, а unlock утримує їх під замком.

Аудит у щоденному дайджесті (рядок `Замок`): скільки рядків під замком, скільки розімкнено за 7 днів, скільки вердиктів пережили свій фікс, скільки позначено `unrescued_at` і скільки розімкнено без старого негативного маркера (`unlockedUnadjudicated7d`, історичний лічильник, не повний облік позитивних доказів). Окремий рядок «Утримано після закриття» показує ID рядків і issues, які втратили відкритий issue, але не мають чинного proof. Це сигнал оператору, що ручне закриття не завершило closeout.
...
| 27 | `enrich_failures.unrescued_at`/`unrescued_issue` (#558) — негативний результат реплею: цей фікс рядок не рятує. Це не термінальний стан і не дозвіл закривати issue. Окремі колонки від `retired_at`; пулів не змінює, рядок лишається зі своїм бекофом. Після #697 безкоштовний ретрай потребує окремого позитивного proof |
...
| 36 | `enrich_failures.rescued_*` (#697) — позитивний per-row proof із bid, входом і lookup-знімком; без бекфілу. Тригер інвалідує його при зміні issue/класу; новий реальний провал інвалідує при upsert. Закриття issue саме не створює proof |
