...
наявний аліас її ключа на цей рядок; синк чекінів, `/import` і пін аліас не переносять.
...
записується на нову сироту картки, і аліас переходить туди разом із доказом (`ensureOrphan` → `applyLookupOutcome`: злиття у власника bid або новий
...
- **Ідентичність, здобута злиттям, переживає повторну ту саму картку крамниці (#614).** Ключ аліасу — усе, що
  ...
  `/enrich/*` для картки з аліасом сироти не створюють.
