...
| 29 | `checkin_coverage` — покриття чекінів діапазонами + сид для користувачів із `synced >= profile_total` (#587) |
...
`checkin_id` **впоперек батчів одного імпорту** (передаються явно від виклику до
виклику), але сам батч **нічого не пише** в `checkin_coverage` (§3.15): партії одного
файлу — суцільні зрізи одного експорту, тож їхнє об'єднання доведене, а от те, що САМ
файл — повна історія без дір, ніщо не перевіряє (обірвана на пів-дороги вигрузка
парситься так само чисто й просто дає менше рядків).
...
`profile_total` відомий і кількість чекінів у користувача (`synced`) уже `≥
profile_total`, той самий лічильник і той самий поріг, яким користується сидування
міграції 29. Немає лічильника, або файл дав менше рядків, ніж лічильник обіцяв, —
покриття не пишеться взагалі, і перший живий обхід проходить цю історію сам. Інакше
непідтверджена заявка могла б мовчки запечатати діру, яку знайшла й записала жива
...
чужої сторінки, тож пара відхиляється до будь-якого запису.
