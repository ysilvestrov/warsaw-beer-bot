...
- джоба `unlock-fixed-orphans` (щогодинний тік, раз на варшавську добу через `job_state`) звіряє `issue_number` заблокованих рядків із набором відкритих issue за лейблом `orphan-triage`. Ті, чий issue **вийшов** із набору, отримують `rearmLookup` (лічильник backoff у 0) і штамп `unlocked_at` — **такт 1**;
...
Аудит у щоденному дайджесті (рядок `Замок`): скільки рядків під замком, скільки розімкнено за 7 днів і скільки вердиктів пережили свій фікс. Нуль розімкнень за тиждень, у якому закривалися issue, означає, що механізм мертвий.
