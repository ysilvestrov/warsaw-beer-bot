...
ABV лише накладає вето. Виняток: якщо в наближеному пулі жоден кандидат не несе `rating_count`
(транспорт без popularity-сигналу — legacy `htmlSearch`-relay, web-фолбек), стадія повертається
до старої поведінки — вибір рівно одним кандидатом у `ABV_TOLERANCE`, як на точному ключі: цей
транспорт лишається без свого єдиного сигналу, тож втрачати ще й ABV-вибір йому не можна.
