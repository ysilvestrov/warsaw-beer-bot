...
**Покриття (#587).** Кожен батч записує в `checkin_coverage` (§3.15) діапазон
`[MIN, MAX]` `checkin_id`, доведений САМЕ цим імпортом. Межі акумулюються **впоперек
батчів одного імпорту** (передаються явно від виклику до виклику) і ніколи не
розширюються проти діапазонів, які вже лежать у таблиці: партії одного файлу — суцільні
зрізи одного експорту, тож їхнє об'єднання доведене; чужий діапазон — ні. Інакше
застаріла вигрузка могла б мовчки запечатати діру, яку знайшла й записала жива
