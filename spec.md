...
- **SQLite busy-handling — багатошарове, не «скрізь обгортати».** Базовий рівень:
  `busy_timeout = 5000` (наразі — неявний дефолт `better-sqlite3` v12.9.0; буде
  явно закріплено в `openDb`, див. план `2026-06-06-explicit-busy-timeout`) →
  будь-який заблокований запис синхронно ретраїться до 5 с на рівні SQLite,
  покриваючи **всіх** писачів (startup-джоби, крони). Другий рівень —
  `withBusyRetry` (експоненційний бекоф)
