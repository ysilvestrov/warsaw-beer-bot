...
- **Застосування** — `parseVerdictFile` + `applyVerdicts` (`src/jobs/adjudicate-apply.ts`), команда `npm run adjudicate -- --apply <файл>`. Мережі не торкається. `parseVerdictFile` розбирає файл суворо (кидає на першій-ліпшій невідповідності форми — краще відмовитись цілком, ніж застосувати половину незрозумілого); `applyVerdicts` звіряє кожен вердикт `unrescued` із поточним рядком (чи ще orphan, чи не ретайрений, чи issue не зрушив, чи brewery/name не переписали) і лише тоді ставить `markUnrescued`, усе в одній транзакції. Що не пройшло звірку — у звіті `skipped` з причиною, не мовчки.
