...
| `abv` | REAL | nullable | міцність, %; заповнюється з Untappd (`refreshAllUntappd` парсить `.abv`, backfill через `COALESCE`; orphan-lookup — теж). Синк чекінів і `/import` на рядку, знайденому за bid, **лише заповнюють порожнє** й нічого не стирають; на резолвленій сироті Untappd-значення переважає (#617) |
| `rating_global` | REAL | nullable | публічний рейтинг Untappd (`global_weighted_rating_score`); синк чекінів і `/import` на рядку, знайденому за bid, лише заповнюють порожнє (#617). `refreshAllUntappd` на рядку, знайденому за bid, **перезаписує** рейтинг значенням зі сторінки `/beers` (зокрема `NULL`, коли Untappd рейтингу не показує) |
