...
Після всіх чинних стадій `lookupBeer` має strict collab-token boundary rescue (#613) для
збереженого магазином учасника колаборації, склеєного в назві пива (`MultiQlti` ↔
`Multi Qlti`). Це не загальна нечутливість до меж слів: один повний нормалізований
...
результат лишається `not_found`. Стадія термінальна, тож не змінює жоден збіг, який уже
прийняла сильніша стадія, і не змінює `normalizeName`, `nameKeys`, fuzzy-пороги чи
popularity-resolver.
