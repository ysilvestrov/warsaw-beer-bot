...
| `rescued_lookup_count`/`rescued_lookup_at`/`rescued_rearm_count`/`rescued_failure_count`/`rescued_probed_at` | INTEGER/TEXT/INTEGER/INTEGER/TEXT | nullable (міграція 36) | знімок lookup, rearm і `enrich_failures.fail_count` та час проби. `fail_count` звіряється при застосуванні, але не при unlock: заблокована мережна спроба теж збільшує цей лічильник, не спростовуючи доказ. Closeout і unlock звіряють lookup/rearm та інші поточні поля; новий реальний провал окремо очищує маркер |
...
- **Проба** — `npm run adjudicate -- --issue <n>` перевіряє канарку до й після, читає нематчені/неретайрені рядки issue та пише файл вердиктів у `/tmp`. Кожен має brewery/name і lookup count/time/rearm count; `rescued` додатково має matched bid, ABV входу (`null` допустимий) та `failure_count`. Провал канарки або відкритий circuit breaker не дає застосовного файлу. Старий `unrescued_at` дає `already_marked` без нового пошуку — це не дозвіл закрити issue; для нового реплею потрібен явний rearm.
- **Застосування** — `npm run adjudicate -- --apply <файл>` суворо розбирає файл (старий `rescued` без bid/ABV/`failure_count` відхиляється), звіряє кожен позитивний і негативний вердикт із поточними issue, brewery/name, ABV для позитивного, lookup count/time і `rearm_count`, а позитивний — також із поточним `fail_count`; потім в одній транзакції пише `rescued_*` або `unrescued_*`. Зрушений рядок отримує явну причину `skipped`; `--force` обходить лише чотиригодинне вікове вікно файлу, не перевірки стану чи заборону часу проби в майбутньому. Позитивний proof інвалідується новим реальним провалом, зміною issue/класу або розбіжністю поточного знімка; активне рішення #695 також забороняє unlock.
