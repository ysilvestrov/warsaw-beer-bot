...
- **Контекст:** `scripts/ai-review/context.ts` шле моделі і діф, і повний HEAD-вміст
  ...
  вичерпається бюджет `CONTEXT_BUDGET` (240 000 символів). Файли, що не влізли,
  явно перелічуються під «Files where you see only the diff» з інструкцією не
  робити тверджень про непоказаний код.
- **Механічний gate (`gate.ts`, чистий, без мережі).** Відкидає зауваження, якщо:
  ...
  normalised) у HEAD-вмісті файлу (`quote_not_found`); знайдена ділянка не
  перетинається зі зміненими рядками діфу (`outside_changed_lines`, з
  урахуванням context-рядків хунка); або воно дублює вже прийняте зауваження за
  файлом+нормалізованою цитатою (`duplicate`). Gate також **виправляє** заявлений
  моделлю номер рядка на реальну позицію збігу — це його найнадійніший внесок
  (див. нижче про виміряну слабкість `quote_not_found`).
- **Лічильники.** Кожне рев'ю несе футер `N raised → M passed the evidence gate →
  K confirmed on review`; якщо жодне зауваження не пережило verify — явний текст
...
- **Fail-семантика:** скрипт сам володіє exit-кодом. Червоно (exit 1): відсутні
  ...
  **верифікації окремого** зауваження знімає лише його (`verdict: 'error'`) і
  ...
  зауважень») або в діфі немає файлів у скоупі (skip із `::notice::`).
- **Offline replay:** `npm run ai-review-replay -- <pr>` ганяє весь пайплайн
  проти вже існуючого PR (діф + HEAD-вміст через `git show`) без постингу — той
  самий інструмент, яким виміряно вибір моделей вище.
