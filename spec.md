...
**Адюдикація (`adjudicateIssueRows`, #558).** Тригер такту 1 навмисно грубий — «issue закрився», не «фікс рятує». Політика вже вимагає живого реплею перед фіксом `parser_bug`/`matcher_bug`; `adjudicateIssueRows` — це місце, куди той реплей пишеться, замість того щоб померти з сесією чату. Проганяє живу пробу по кожному рядку, що називає issue (виключно нематчені й нередайрені — `retired_at IS NULL AND untappd_id IS NULL`), і завершує рядок в одному з трьох станів: пробу **знайдено** — рядок лишають як є, крон його злінкує; пробу **не знайдено** (`not_found`) — `markUnrescued` ставить `unrescued_at`/`unrescued_issue`; проба **transient/blocked** — не пишеться нічого, мережевий збій не вердикт (той самий принцип, що й у #316). Рядок за рядком — інструмент свідомо не має гуртового шляху `WHERE issue_number = …`. `unrescued_at`/`unrescued_issue` знімаються (`clearUnrescued`) будь-яким явним `rearmLookup`: маркер не печатка, а твердження «на сьогодні задарма не купиш», яке нова проба спростовує. Свідомо без наслідків для каталогу — не пише лінк на матч, не рухає бекоф на `not_found` — щоб одна помилка в інструменті адюдикації не псувала БД так, як зробила б помилка в кроні.
