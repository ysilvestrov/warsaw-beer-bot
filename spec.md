...
| 33 | `match_links` перебудовано з ключем `UNIQUE (ontap_ref, brewery_ref)` (#632). Наявні лінки розкладено за броварнями зі збережених знімків: одна броварня — пара; пін лишається, лише якщо назва є в знімку всередині вікна збереження (останній знімок − 14 днів), штамп — лише якщо `merged_at` у цьому вікні (поза ним `cleanup-old-snapshots` лишає тільки останній знімок паба, і свідок злиття міг зникнути); кілька броварень — копія на пару без піна й штампа; жодної — видалено. Dry-run справжнім кодом на байтовій копії прод-БД: 5411 → 1685 лінків, 7 пінів, 20 штампів; показ 1692 кранів і множина «на крані» (932) без змін |
...
- **Лінк крана ключується парою точного тексту броварні й назви крана (#632).** Кран однієї броварні ніколи не
  ...
  з'єднують кран із лінком за цією парою.
