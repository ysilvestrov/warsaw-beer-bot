...
**Запис** — новий аліас створює лише `mergeIntoCanonical`, у транзакції до `DELETE`; наявний переносять `ON CONFLICT`
