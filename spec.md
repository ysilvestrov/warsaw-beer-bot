...
### 3.16 `api_usage` — денний облік запитів розширення (v17)
| Поле | Тип | Обмеження | Опис |
|------|-----|-----------|------|
...
**Покриття (#587).** Кожен батч зливає рядки й накопичує межі `[MIN, MAX]`
`checkin_id` (лише чисто десяткові id, `/^\d+$/` — рев'ю PR #592, P2: без цього `Number()`
пропустив би `'5e2'`, `' 580 '` чи `'0x244'` як валідні числа, хоча рядок зберігається в БД
під своїм ЛІТЕРАЛЬНИМ рядковим id, і межі посунулися б за числом, якого насправді немає)
разом із лічильником `rows` — скільки таких рядків дав файл, — **впоперек батчів одного
імпорту** (передаються явно від виклику до виклику), але сам батч **нічого не пише** в
`checkin_coverage` (§3.15): партії одного файлу — суцільні зрізи одного експорту, тож їхнє
об'єднання доведене, а от те, що САМ файл — повна історія без дір, ніщо не перевіряє
(обірвана на пів-дороги вигрузка парситься так само чисто й просто дає менше рядків).

Заявку про покриття робить окремий крок, рівно один раз — коли файл вичерпано і всі
батчі злиті: `[MIN, MAX]` пишеться, тільки якщо зовнішнє свідчення це підтверджує —
`profile_total` відомий і **рядків, які дав САМ файл** (`bounds.rows`), уже `≥
profile_total`. **Рев'ю PR #592 (P1): звірка йде саме з `bounds.rows`, не з
`countCheckins`/`synced`** — той рахує УСІ рядки користувача з будь-якого джерела, тож 8
рядків, уже злитих живою синхронізацією, плюс 2 з обірваного імпорту дали б `countCheckins =
10 = profile_total` і хибно "підтвердили" заявку, хоча файл насправді покрив лише 2 з тих 10.
Немає лічильника, або файл дав менше рядків, ніж лічильник обіцяв, — покриття не пишеться
взагалі, і перший живий обхід проходить цю історію сам. Інакше непідтверджена заявка могла б
мовчки запечатати діру, яку знайшла й записала жива
синхронізація десь-інде — саме ту хворобу, з якої почався #587.
