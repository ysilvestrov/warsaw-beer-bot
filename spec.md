...
Авторизація **обов'язкова** (`authMiddleware`), анонімного шляху немає — на відміну
від `/match` (#245). Причина не безпекова: анонімна відповідь має `is_drunk: false`
...
`src/api/routes/mcp.test.ts`). Виняток — перевищення ліміту тіла: його ловить
`payloadBodyLimit` ще до SDK, тож воно приходить як HTTP-413, не в JSON-RPC-формі.
