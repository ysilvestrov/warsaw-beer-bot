...
- **Три детерміновані гейти вердикту** в `planTriageActions` (чиста функція, без
  ...
  2. Вердикт приліплюється до відкритої issue, лише якщо рядок задовольняє її scope.
     ...
     купу. **Відмова не викидає класифікацію (#509).** Раніше відмовлений вердикт ставав
     `skipped` — рядок лишався нетріаженим і повертався в завтрашній батч, де модель
     робила той самий вибір і отримувала ту саму відмову (на проді — 9 днів поспіль той
     самий рядок на ту саму issue). Тепер клас **зберігається**, ціль (`issue_number` /
     `new_issue_key`) знімається, а `review_note` стає
     `off-scope <ціль>: <причина> | <оригінальна нотатка моделі>` (перший term, якому рядок
     суперечить, напр. `candidates_count = 0`; якщо в issue взагалі немає блоку scope —
     літерально `no scope block`, а не «outside the cohort», бо рядок не провалив належність
     до когорти — в issue просто нема scope). Складений рядок обрізається до 500 символів
     власним `.slice(0, 500)` у `refuseRoute` — `VerdictSchema` тут ні до чого, вона
     обмежує лише вхідну нотатку моделі ще до склеювання.
     Оригінальна репліка моделі («shop brand in brewery field» тощо) навмисно **не**
     викидається: для рядка, чия єдина мета після відмови — чекати людину, це єдина заява
     про те, чим дефект насправді є. Рахується окремо від `guardHits.scope_violation` (той
     лічить спрацювання гейта, безумовно) як `quietOffScope` (`planTriageActions`) →
     `outcome.offScope`, і публікується в дайджесті як `N без власника (поза scope)`. Гейт
     **виключає `not_a_beer`** з цього лічильника (той самий виняток, що й у
     `quietCauseStripped`/`quietNoTarget` вище, #432): `not_a_beer` уже має власний
     `outcome.notABeer` і свою частину дайджесту, тож рахувати його ще й тут — подвійний
     облік того самого рядка.
...
- **Триаж-інбокс (#509).** Рядки, чию ціль відмовив scope-гейт вище, не зникають з
  ...
  відмовленою ціллю (номером issue чи ключем пропонованої нової), у заголовку —
  ...
  побачивши жодної відкритої issue з міткою `triage-inbox`, заводить свіжу.
