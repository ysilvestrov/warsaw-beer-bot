...
│   ├── drink-boundary.ts   # спільна межа «напій, який шукаємо на Untappd» — не приватний ontap-гейт: ontap tap-exclusion + пост-пошуковий enrich-enforcer (#430)
...
| `abv` | REAL | nullable | міцність, %; заповнюється з Untappd (`refreshAllUntappd` парсить `.abv`, backfill через `COALESCE`; orphan-lookup — теж). Гідратор (#616) лише заповнює порожнє. Синк чекінів і `/import` на рядку, знайденому за bid, **лише заповнюють порожнє** й нічого не стирають; на резолвленій сироті Untappd-значення переважає (#617) |
| `rating_global` | REAL | nullable | публічний рейтинг Untappd. **Нуль не пишеться** (#616): Untappd не показує рейтинг пива з менш ніж 10 оцінками (Algolia `rating_score: 0`, сторінка «Global Rating (N/A)»), тож усі входи рейтингу **Untappd** (Algolia-пошук і `getObjects`, relay HTML-пошук, експорт `/import`, сторінка `/beers`) проходять через `untappdRating`, яка дає `NULL` і округлює до 2 знаків; наявні нулі перевела в `NULL` міграція 31. Виняток — `u_rating` з ontap: сирота отримує його з крана (`ensureOrphan`), лінкування зберігає його через `COALESCE`, доки гідратор не звірить рядок (нулів ontap не віддає — виміряно). Значення, записані до #616, можуть мати більше знаків, доки їх не переписав перший прохід гідратора. Синк чекінів і `/import` на рядку, знайденому за bid, лише заповнюють порожнє (#617). Гідратор `hydrateRatings` перезаписує рейтинг значенням Algolia (зокрема `NULL`); `refreshAllUntappd` на рядку, знайденому за bid, перезаписує його лише коли блок «Global Rating» на картці знайдено (число або «N/A») |
...
| `rearm_count` | INTEGER | NOT NULL DEFAULT 0 (міграція 28) | скільки разів рядок явно ре-армили. **Монотонний і ніколи не обнуляється** — на відміну від двох полів вище, які `rearmLookup` якраз і скидає в нулі. Потрібен саме тому: рядок, щойно ре-армлений і ще не перепробуваний, уже має нулі, тож наступний ре-арм не змінює в БД нічого, і адюдикація (#576) не побачила б, що між пробою і застосуванням рядку дали новий шанс. Лічильник, а не таймстамп: два ре-арми в ту саму мілісекунду таймстамп не розрізнив би |
...
| `rating_refresh_count` | INTEGER | NOT NULL DEFAULT 0 (v6) | скільки разів поспіль Algolia не знала bid; розклад `lookup-backoff` (0/72/168/728 год), після вичерпання рядок не звіряється. Успішна звірка гідратором і штамп `refreshAllUntappd` скидають обидва поля |
...
**Ключ — пара броварні й назви крана (#632).** До v33 лінк ключувався самою назвою, і крани різних броварень з
однаковою назвою («Hefeweizen» Friedenfelser і Rittmayer) ділили один рядок: паб, де матчер влучав, переписував лінк
і стирав штамп злиття, паб, де промахувався, щоцикла створював і зливав сироту, а показ пабу брав пиво останнього паба
циклу. Ключ — точний текст без канонізації: з'єднання `taps` ↔ `match_links` іде в SQL (показ, #486), а `ontap_ref` і
до того розрізняв регістр. Та сама броварня з іншим написанням (`PINTA` / `PINTA Brewery`) — окремі пари; матчер
зводить їх до того самого пива.
...
(`--beer/--untappd` для піна, `--unpin --ref [--brewery]/--beer`, `--list`). Два випадки на рівні даних:
...
`untappd_id` виставляється на власному рядку orphan'а. Пін ключується на парі точного тексту броварні й
назви крана (`brewery_ref`, `ontap_ref`): якщо магазин згодом перевикористає **той самий** рядок для **іншого**
...
|---|---|---|---|
| `id` | INTEGER | PK AUTOINCREMENT | |
| `beer_id` | INTEGER | NOT NULL → `beers(id)` ON DELETE CASCADE | рядок, який довів найновіший доказ для цієї картки (злиття чи лінк) |
...
**Запис** — новий аліас створює `mergeIntoCanonical` (або доказовий ручний ремонт §3.6.2), у транзакції до `DELETE`; наявний переносять `ON CONFLICT`
злиття і `recordLookupSuccess`. Картка — вхід `applyLookupOutcome`: шлях bid —
картка з тіла запиту; шлях пошуку — текст картки й ABV, з яким ішов пошук (`row.abv`); крон — сама сирота.
Аліас пишеться, **лише коли злита сирота і є цією карткою** (той самий `cardText` броварні й назви), — крім
доказу опублікованим bid: він узятий лише з полів картки, тож написання сироти не важить; для порожнього тексту не
пишеться. `ON CONFLICT` переводить ключ на новий рядок. Зміна `untappd_id` рядка
(`recordLookupSuccess`, `pinMatch`, `upsertBeerByBid` для рядка без bid) видаляє його аліаси: вони доводили старий
bid. Лінк рядка самої картки новішим доказом (`recordLookupSuccess` з карткою: `/enrich/result`, крон) переносить
наявний аліас її ключа на цей рядок (на шляху bid — на будь-який рядок, на який ліг bid картки); синк чекінів, `/import` і пін аліас не переносять.

**Читання.** `/match` і MCP перевіряють аліас **перед** матчером (див. `POST /match`). Аліас відповідає
за свій точний ключ завжди: конфлікт доказів для тієї самої картки розв'язує запис, а не читання.
...
### 3.6.2 `legacy_card_repairs` — аудит доказового ремонту старої картки (v34, #696)

Після виправлення адаптера стара версія розширення може й далі надсилати помилкову картку. Якщо оператор
довів, який саме Untappd-bid ця картка означала, він може злити її сироту з канонічним пивом і зберегти
точний старий ключ у `beer_aliases`. Кандидат пошуку сам по собі не доказ: потрібен зв'язок старої картки
з товаром, доказ ABV, яке надсилав старий клієнт, і пряме посилання товару на bid або документований
незалежний ланцюг доказів. Без цього рядок лишається невирішеним.

Ремонт — одна операторська дія над одним `beer_id` і його `issue_number`, із dry-run за замовчуванням.
Перед записом він знову перевіряє стан сироти, issue, поля картки, лукап-стан і точний ключ аліаса.
Чужий bid на тому самому ключі — конфлікт, а не привід перевести аліас через `ON CONFLICT`.
Новий канонічний рядок для відсутнього bid створюється без евристики всиновлення сироти за назвою.
Злиття посилань, перенесення `untappd_had`, старий аліас, видалення сироти та журнал рішення — одна
транзакція; помилка не залишає часткового ремонту. Повтор тієї самої вже завершеної дії — no-op лише
коли журнал і чинний аліас підтверджують той самий bid.

Ключ аліаса містить **ABV старої картки**, навіть якщо магазин помилився. Якщо це значення або вже записане
ABV канонічного пива відрізняється від гідрованого Untappd-ABV, запис вимагає явного
`--overwrite-abv`: канонічне `beers.abv` стає значенням Untappd, а старе лишається в ключі та аудиті.
За відсутності гідрованого ABV конфлікт не переписується. Без різниці флаг відхиляється як зайвий.

`legacy_card_repairs` — незмінний знімок рішення, не зовнішній ключ до рядка, який видаляється:
`orphan_beer_id` (UNIQUE), `issue_number`, сирі `card_brewery`/`card_name`/`card_abv`,
`failure_source_url`, `target_bid`, `canonical_beer_id`, `evidence_url`, `operator`, `reason`,
`overwrite_abv`, `prior_canonical_abv`, `final_canonical_abv`, `applied_at`. Обидва локальні ID —
історичні знімки: подальше видалення чи заміна канонічного рядка не стирає причину ремонту.
Звичайне автоматичне перенесення аліаса не змінює цей журнал. `enrich_failures.review_note` для цього
не годиться, бо його рядок видаляється після успішного злиття.

### 3.6.3 Неактивна стара сирота (#695)

Коли адаптер уже виправлено, але конкретну стару картку неможливо доказово пов'язати з
Untappd-bid, оператор може вивести **одну сироту й один точний ключ старої картки** з
автоматичної роботи. Це окремий стан, а не `retired_at` («фікс розв'язав») і не
`unrescued_at` («цей реплей не врятував»). `review_class` та `unrescued_at` не змінюються.
Порожній результат пошуку сам по собі не доводить, що ідентичність товару невідома.

`legacy_orphan_dispositions` зберігає історичний `beer_id`, `issue_number`, сирі
brewery/name/ABV старої картки, точний ключ `cardText`/`cardAbv`, `source_url`,
причину, URL доказів, оператора й час рішення. ABV картки (або його доведену
відсутність) оператор встановлює з історичного джерела; поточне `beers.abv` не є
доказом того, що надіслав старий клієнт. Активний запис унікальний для `beer_id` і
точного ключа картки. Рішення лишається в аудиті після видалення сироти; активний
запис може закрити **лише явна операторська дія** з новою причиною, доказом,
оператором і часом. Повторна неактивація створює новий історичний епізод.
Автоматичний lookup, скидання review, закриття issue, оновлення ABV чи поява bid
ніколи не відкривають його. Операторська команда працює по одному `beer_id`, має
dry-run за замовчуванням, перед записом транзакційно перевіряє живу сироту,
issue/картку, відсутність конфліктного аліаса та іншого активного рішення; stale
або суперечливий стан відхиляє без часткового запису.

Поки рішення активне, `beer_id` відсутній в обох фонових enrich-пулах, relay-черзі,
лічильниках активних черг, web fallback, щоденному тріажі та issue-close rearm.
`/match` (і MCP) не дають матчу для його **точного** ключа й не повертають
неактивний рядок навіть зі старого знімка каталогу. `/enrich/candidates` відповідає `eligible: false`,
а прямий `/enrich/result` — наявним `not_found`, навіть із bid від старого клієнта;
обидва перевіряють ключ до створення/оновлення сироти. Автоматичне повторне
використання за нормалізованою назвою та всиновлення сироти за bid оминають
неактивний рядок. Lookup, який почався до операторського рішення, повторно
перевіряє стан під блокуванням запису й не змінює рядок; ручні pin/repair
вимагають попереднього явного reopen. Інший, правильно розпарсений ключ іде звичайним шляхом;
відмінний ABV — відмінний ключ, а глобальної заборони назви немає. Якщо новий
клієнт надсилає той самий точний ключ, сервер не може розрізнити версії й
блокує його до ручного відкриття. Ручне відкриття саме по собі не скидає backoff
і не стверджує bid.
...
| `issue_number` | INTEGER | nullable (міграція 23) | GitHub-issue, до якої тріаж прив'язав цей рядок. **Авторитетне** джерело зв'язку: суфікс `→ #N` у `review_note` пишеться далі, але лише для людського ока. Читає звіт про насичення (#431) через `countRowsForIssue(db, issue, sinceIso)`, який рахує рядки, дописані **після** створення issue |
| `unlocked_at` | TEXT | nullable (міграція 25) | час, коли рядок отримав **безкоштовний ретрай** після того, як його issue вийшов із набору відкритих (#421). Не «розблоковано назавжди», а «ставка в польоті»: вердикт при цьому **зберігається**, бо ми його не спростували, а перевіряємо. Скидається в `NULL`, щойно ставка закривається — успіхом (рядок зникає) або новим провалом (вердикт знімається) |
...
| `unrescued_at` | TEXT | nullable (міграція 27) | третій термінальний стан (#558), протилежний `retired_at`: не «фікс розв'язав», а «реплей на цьому фіксі довів, що ЦЕЙ рядок він не рятує». Ставить `markUnrescued` (з `applyVerdicts`, `src/jobs/adjudicate-apply.ts` — #576), ідемпотентно (`WHERE unrescued_at IS NULL`). Пулів не змінює — рядок лишається в пулі зі своїм наявним бекофом; єдиний ефект: `unlock-fixed-orphans` знімає замок (`unlocked_at`), але більше не дає безкоштовне `rearmLookup`. Знімається (разом з `unrescued_issue`) будь-яким явним `rearmLookup` — маркер **не печатка**, а «на сьогодні задарма не купиш» |
| `unrescued_issue` | INTEGER | nullable (міграція 27) | issue, чий реплей довів `unrescued_at` — машиночитана причина окремо від `review_note`, щоб її можна було запитати, а не парсити текст |
...
|---|---|---|---|---|---|---|
| 1 | Це взагалі позиція пива? | `not_a_beer` | сам товар (мерч, скло, вино/коктейль/їжа, бандли/мультипаки/подарункові набори — набір не є пивом, навіть якщо кожна пляшка в ньому пиво) | фільтр інжесту | **так — єдиний клас, що виключає** | ні, і це правильно |
...
Рядок 1 навмисно **не** перелічує cider/kvass/mead/kombucha як не-пиво: це eligible-родини, які Untappd каталогізує і які наш матчер резолвить щодня. Єдине джерело цього переліку — `ELIGIBLE_TOKENS` у `src/domain/drink-boundary.ts` (`cydr`, `cider`, `kwas chlebowy`, `kvass`, `квас`, `mead`, `melomel`, `kombucha`); тріаж-промт не тримає власної копії, а читає той самий масив через `eligibleFamiliesForPrompt()`. Доказ, що це не косметика: власний каталог тримає **1339** зматчених рядків зі стилем Cider/Mead/Kvass і **10** — зі стилем Kombucha (`Hard Kombucha / Jun`, `Non-Alcoholic - Kombucha`).
...
- **`unidentifiable` — твердження про нашу здатність розрізняти СЬОГОДНІ**, а не про світ: «кілька кандидатів, не вибрати» усувається тай-брейком (#409), «вигадана броварня» — новим аліасом (#347/#327). Тому сам клас **не** виключає рядок із пулів: інакше наявний авто-розпечатувач (`recordEnrichFailure` скидає клас, коли `candidates_count` перетинає межу `0 ↔ >0`) не може спрацювати ніколи — печатка тримає рядок подалі від лукапу, який її ж і зняв би. Окреме доказове операторське рішення #695 (§3.6.3) є винятком, не новим значенням класу. Складність фіксу ніколи не є входом класифікації: «я знаю, що це Guinness, але наші правила його не дістають» — це `matcher_bug`.
...
**Замок за фіксом (`#421`).** Розклад backoff — це ставка на те, що відповідь змінює **час**. Для `not_on_untappd` вона істинна (каталог Untappd росте), для дієвих класів — хибна: поки баг не пофікшено, відповідь завтра така сама, як сьогодні. Тому:

- рядок із класом `matcher_bug`/`parser_bug`, ненульовим `issue_number` і порожнім `unlocked_at` **не потрапляє в жоден enrich-пул** (клауза `lockedRowPredicate` у `listLookupCandidates` і `listRelayLookupCandidates`). Усі три умови обов'язкові: вердикт без issue не може бути розімкнений нічим, тож замок на ньому був би вічною печаткою;
- джоба `unlock-fixed-orphans` (щогодинний тік, раз на варшавську добу через `job_state`) звіряє `issue_number` заблокованих рядків із набором відкритих issue за лейблом `orphan-triage`. Ті, чий issue **вийшов** із набору, отримують `rearmLookup` (лічильник backoff у 0) і штамп `unlocked_at` — **такт 1**. Виняток (#558): якщо рядок несе `unrescued_at`, `rearmLookup` пропускається (`rearmSkipped` у результаті джоби) — `unlocked_at` усе одно ставиться, замок усе одно знято, але безкоштовне обнулення бекофу рядку, чий фікс реплей уже довів, що не рятує, купувало б лише лукапи, які не можуть влучити;
- **такт 2** настає в `recordEnrichFailure`: провал ретраю на рядку з `unlocked_at` знімає `review_class`/`review_note`/`reviewed_at` і сам `unlocked_at`, повертаючи рядок у чергу тріажу. `issue_number` **зберігається** — це слід того, який саме фікс перевірили й не покрив. Через порядок із гейтом `blocked` (див. вище) збій Untappd ставку не закриває: він до цього стейтмента не доходить;
- тригер навмисно грубий — «issue більше не відкритий», а не «фікс приїхав»: GitHub другого сказати не вміє (`closedByPullRequestsReferences` порожній, `stateReason` відображає натиснуту кнопку, а не код). Ціна помилки — **один** лукап на рядок проти чотирьох, які палив таймер;
- замок **не діє** на `/enrich/candidates`: той пошук іде сесією користувача, а не нашою квотою. Рекурентний хвіст backoff діє там і там;
- `not_on_untappd` не має термінального стану: після вичерпання розкладу останній крок (728 год) **повторюється**, бо клас за побудовою зворотний. `unidentifiable` і legacy-рядки без issue лишаються з термінальним розкладом — у них нема ні власника фікса, ні зовнішнього каталогу, на який чекати.

**Адюдикація (#558, #576).** Тригер такту 1 навмисно грубий — «issue закрився», не «фікс рятує». Політика вже вимагає живого реплею перед фіксом `parser_bug`/`matcher_bug`; адюдикація — це місце, куди той реплей пишеться, замість того щоб померти з сесією чату. Розведена на дві фази не заради тривалості транзакції — інлайнова версія з #558 такої проблеми не мала, вона писала по рядку одним стейтментом, без відкритої транзакції на час мережевих викликів. Справжня причина — захист від масового хибного вердикту: жоден маркер не сміє з'явитися, поки канарка не пройшла **з обох боків усього прогону**, а єдиний спосіб це гарантувати — не писати нічого, доки прогін (усі рядки issue) не завершився цілим. Розділення на «проба, яка нічого не пише» і «застосування, яке звіряє й пише» — механізм цієї гарантії: файл вердиктів народжується лише після закривної канарки, тож `--apply` отримує або вердикти прогону, що довів свою чесність цілком, або не отримує файлу взагалі. Побічна вигода — запис справді стає короткою транзакцією без мережі всередині — реальна, але вона не була причиною розділення:

- **Проба** — `probeIssueRows` (`src/jobs/adjudicate-issue-rows.ts`), команда `npm run adjudicate -- --issue <n>`. Нічого не пише. Перевіряє канарку (`CANARY_QUERY`) до і після прогону; якщо канарка провалилась з будь-якого боку, увесь прогін відкидається цілком (`{status: 'canary_failed', at}`) — часткового результату не буває, бо системний збій пошуку (протухлий ключ, перейменований індекс, м'який бан) інакше проявився б як `unrescued` на кожному рядку issue. Circuit breaker лише читає: розімкнений — відмова (`{status: 'circuit_open'}`) без спроби пробити, ніколи не пише в нього сам. Інакше проганяє живу пробу по кожному рядку, що називає issue (виключно нематчені й нередайрені — `retired_at IS NULL AND untappd_id IS NULL`), і друкує **файл вердиктів** (`VerdictFile`: `{ issue, probed_at, verdicts: [{ beer_id, brewery, name, verdict, lookup_count, lookup_at, rearm_count }] }`, де `verdict` — `unrescued`/`rescued`/`inconclusive`/`already_marked`, а `lookup_count`/`lookup_at` — стан лукап-бухгалтерії рядка на момент проби) у `/tmp`.
- **Застосування** — `parseVerdictFile` + `applyVerdicts` (`src/jobs/adjudicate-apply.ts`), команда `npm run adjudicate -- --apply <файл>`. Мережі не торкається. `parseVerdictFile` розбирає файл суворо (кидає на першій-ліпшій невідповідності форми — краще відмовитись цілком, ніж застосувати половину незрозумілого); `applyVerdicts` звіряє кожен вердикт `unrescued` із поточним рядком (рядок узагалі існує, чи ще orphan, чи не ретайрений, чи issue не зрушив, чи brewery/name не переписали, чи не зрушив лукап-стан, чи не зріс `rearm_count`) і лише тоді ставить `markUnrescued`, усе в одній транзакції. Що не пройшло звірку — у звіті `skipped` з причиною (`missing`/`not_orphan`/`retired`/`issue_moved`/`input_changed`/`lookup_moved`), не мовчки. Перед записом друкується сам файл — issue, `probed_at`, вік, розклад вердиктів за типом — і застосування файлу старшого за `STALE_VERDICT_FILE_MS` (4 год) відмовляється без `--force` (#576 I3). Ре-арм, що стався між пробою і застосуванням, ловить перевірка `lookup_moved`, і вона дивиться на **три** поля. `untappd_lookup_count`/`untappd_lookup_at` ловлять роботу крона і той ре-арм, який справді щось обнулив; `rearm_count` (монотонний, міграція 28) ловить сам **факт** ре-арму навіть тоді, коли обнуляти вже не було чого — рядок, щойно ре-армлений і ще не перепробуваний, має нулі, тож без лічильника наступний ре-арм по ньому не змінював би нічого спостережуваного. Незбіг у будь-якому з трьох означає, що рядок здобув нове свідчення і наша проба вже не найсвіжіша. Вікно свіжості цього не замінює й ним не замінюється: воно судить прогін цілком, а зрушити може окремий рядок усередині цілком свіжого файлу.

Рядок за рядком — інструмент свідомо не має гуртового шляху `WHERE issue_number = …`. `unrescued_at`/`unrescued_issue` знімаються (`clearUnrescued`) будь-яким явним `rearmLookup`: маркер не печатка, а твердження «на сьогодні задарма не купиш», яке нова проба спростовує. Свідомо без наслідків для каталогу — не пише лінк на матч, не рухає бекоф на `not_found` — щоб одна помилка в інструменті адюдикації не псувала БД так, як зробила б помилка в кроні.

Аудит у щоденному дайджесті (рядок `Замок`): скільки рядків під замком, скільки розімкнено за 7 днів, скільки вердиктів пережили свій фікс, скільки позначено `unrescued_at` (`unrescuedRows`) і скільки розімкнено за 7 днів без адюдикації (`unlockedUnadjudicated7d` — рядок і досі сирота, і `unrescued_at` на ньому нема). Останнє — міра дотримання самого правила: адюдикацію застосовують — цифра мала; тихо пропускають — росте. Нуль розімкнень за тиждень, у якому закривалися issue, означає, що механізм мертвий.

**Retirement (`retired_at`).** Класифіковані рядки провалів залишаються в БД назавжди (видаляються лише на `matched` або при резолвленні сироти за bid, #617), тож уже вирішені кластери (вино/спирт тепер фільтруються, brewery=name #238 тощо) продовжують виглядати «активними» й роздувати лічильник orphan'ів. `retire-resolved-orphans` (npm-скрипт, dry-run за замовчуванням, `--apply`) переводить **доказово вирішені** рядки в `retired_at`. Вибір — лише верифікований, ніколи за віком чи класом: **авто-шлях** бере класифікованих orphan'ів, яких поточний `isOntapNonBeerTap` тепер відкидає (доказ вирішення); **escape-hatch `--ids <csv> --reason "<text>"`** — явно задані оператором `beer_id` (для фіксів, яких предикат не ловить — напр. `VINO KARPATIA` italian-`vino`, або parse-split кластери після шипу фіксу). Retired-рядки виключені з обох enrich-пулів (`listLookupCandidates` і `listRelayLookupCandidates`, поряд із `not_a_beer`) та з лічильника `orphansPending` у щоденному дайджесті.

**Хто що пише:** `applyLookupOutcome` (спільний для серверного крона і client-relay)
upsert'ить рядок на `not_found`/`blocked` і **видаляє** його на `matched`; резолвлення сироти
за bid (`upsertBeerByBid` — синк чекінів, `/import`, `refreshAllUntappd`, #617) теж видаляє її рядок,
бо `listUntriagedFailures`/`listLockedRows` не фільтрують `untappd_id IS NULL`. Один рядок на
...
**Пост-пошуковий enforcer (`classifyOrphanAsNonBeer`, #430).** Крім ingest-фільтра (таблиця вище) межа «напій, який шукаємо на Untappd» перевіряється й ДРУГИЙ раз — уже після невдалого пошуку. `classifyOrphanAsNonBeer` (`src/domain/drink-boundary.ts`) викликається з гілки `not_found` у `applyLookupOutcome` (`src/domain/lookup-outcome.ts`), тобто ПІСЛЯ провального Untappd-пошуку, і тому діє однаково і на серверний enrich-крон, і на client-relay `/enrich/result` — обидва йдуть через `applyLookupOutcome`. Три необхідні умови, усі разом: (1) `candidates_count = 0`; (2) жодна eligible-родина (`ELIGIBLE_TOKENS`, див. вище) не згадана ніде в brewery/name/style — підрядком; (3) не-пивний токен (`NON_BEER_NAME_TOKENS`) знайдений на межі слова, а не підрядком. Наразі модуль живе в **shadow-режимі** (`SHADOW_ONLY = true`): при спрацюванні він лише пише в лог `drink-boundary: would classify as not_a_beer` і нічого не записує в `enrich_failures`. Увімкнення (`SHADOW_ONLY = false`) — окрема свідома зміна, після тижня звірки shadow-логу з власними вердиктами LLM-тріажу. Список `NON_BEER_NAME_TOKENS` навмисно короткий: кожен токен, що лишився, перевірений на межі слова проти всіх 31224 зматчених пив каталогу і не влучає в жодне з них як ціле слово (детальніше — коментар над константою в `drink-boundary.ts`, і design-документ #430).
...
| `updated_at` | TEXT | NOT NULL DEFAULT CURRENT_TIMESTAMP | |
| `profile_total` | INTEGER | nullable | останній відомий загальний лік чекінів у профілі Untappd (парситься з кожної сторінки extension-синхронізації; *latest non-null wins*). Використовується `/status` і stop-логікою `POST /checkins/sync` (§4) для порівняння `serverCount` із заявленою повнотою. NULL для користувачів, що не користуються розширенням (import-only / link-only). |
...
До #587 ця таблиця сама була джерелом курсора (`deepest_max_id` як мінімум із двох
не-суцільних обходів) — це стверджувало покриття, якого ніхто не встановлював, і саме так
41 чекін користувача лишився недосяжним в дірі всередині історії, яку жоден з обходів більше
не бачив. Тепер єдине, що тут насправді пишеться — `profile_total`; курсор і факт покриття
живуть у `checkin_coverage` (§3.15).

### 3.15 `checkin_coverage` — доведені діапазони чекінів (v29, #587)
| Поле | Тип | Обмеження | Опис |
|------|-----|-----------|------|
...
Діапазон доводить **сама сторінка фіду**, і нічого більше: фрагмент, запитаний із курсором
`M`, повертає все, що лежить нижче `M`, до свого найстарішого елемента — отже сторінка
доводить рівно `[найстарший_на_сторінці, M]`, незалежно від того, з якого прогону вона
прийшла і чи той прогін обірвався. Це замінює скалярний `deepest_max_id`-мінімум із двох
не-суцільних обходів (§3.14), який міг мовчки ствердити покриття діри, яку жоден обхід
насправді не бачив.

**Міграція 29 не сидує нічого (рев'ю PR #592, P1).** Перша версія писала `[MIN, MAX]` для
кожного, у кого `COUNT(*) >= profile_total` — але рівність лічильників не доводить
суцільність: користувач, що тримає застарілий рядок (чекін, видалений на Untappd, лишився в
нас), може задовольнити лічильник, поки ІНШИЙ id усередині того самого діапазону насправді
відсутній. Сид тоді ствердив би покриття рівно тієї діри, яку застарілий рядок маскував, —
той самий дефект, проти якого ця гілка, лише в новій таблиці. Доказу суцільності для
історичних рядків нема, тож міграція створює порожню таблицю: перший живий обхід кожного
користувача проходить його історію сам і саме цим заодно знаходить будь-яку діру, яку сид
замаскував би.

### 3.16 `api_usage` — денний облік запитів розширення і MCP (v17, MCP-колонки v30)
| Поле | Тип | Обмеження | Опис |
|------|-----|-----------|------|
...
Інкрементується best-effort у `POST /match` і в хендлері тули `match_beers`
(`POST /mcp`); помилка запису не валить відповідь. Один рядок на добу (обмежене
зростання, без cleanup).

`anon_requests`/`authed_requests`/`beers` лишаються трафіком **розширення** — MCP до
них не торкається навмисно. Спільний лічильник зберіг би заяву про розширення, тихо
змінивши те, що вона рахує (розширення ПЛЮС агентів), і виявити підміну не було б
звідки: число просто виросло б. Для MCP-каналу `authed` не має сенсу взагалі — у
`/mcp` анонімного шляху немає, — тож у `mcp_*` він не згортається.

### 3.17 `schema_version` — версія міграцій
Єдине поле `version INTEGER PRIMARY KEY`; по рядку на застосовану міграцію.

### 3.18 Зв'язки (ER, текстом)
```
...
| 23 | `enrich_failures.issue_number` — зв'язок рядка з issue, який доти жив лише текстовим суфіксом `→ #N` у `review_note` (незапитуваним, і вже зіпсованим нотатками про перемаршрутизацію). Бекфіл із суфікса; спершу читався гейтом насичення (#408), з #431 — звітом про насичення |
...
| 25 | `enrich_failures.unlocked_at` (#421) — «безкоштовний ретрай у польоті». Проста `ALTER TABLE ADD COLUMN` без перебудови; має йти **після** міграції 24, бо та перебудовує таблицю за фіксованим списком колонок і тихо викинула б нову |
...
| 27 | `enrich_failures.unrescued_at`/`unrescued_issue` (#558) — третій термінальний стан: реплей при фіксі довів, що ЦЕЙ рядок фікс не рятує. Окремі колонки від `retired_at`, що стверджує протилежне («фікс розв'язав»), — змішування зробило б сторожа сліпим. Пулів не змінює: рядок лишається в пулі зі своїм бекофом, забирається лише безкоштовне обнулення лічильника |
| 28 | `beers.rearm_count` (#576) — монотонний лічильник, що робить сам ФАКТ ре-арму спостережуваним незалежно від того, що він обнулив |
| 29 | `checkin_coverage` — покриття чекінів діапазонами (#587). Без сиду (рев'ю PR #592): `COUNT(*) >= profile_total` не доводить суцільність історії, тож таблиця створюється порожньою — кожен користувач проходить свою історію першим живим обходом |
| 30 | `api_usage.mcp_requests`/`mcp_beers` — окремий облік MCP-каналу. Прості `ALTER TABLE ADD COLUMN` без перебудови (у таблиці нема жодного `CHECK`). Окремі колонки, а не спільні з наявними: §3.16 оголошує наявні лічильники трафіком розширення, і дайджест друкує з них рядок саме про розширення |
...
| 33 | `match_links` перебудовано з ключем `UNIQUE (ontap_ref, brewery_ref)` (#632). Наявні лінки розкладено за броварнями зі збережених знімків: одна броварня — пара; пін лишається, лише якщо назва є в знімку всередині вікна збереження (останній знімок − 13 днів — на день менше `SNAPSHOT_RETENTION_DAYS`, бо `cleanup-old-snapshots` ріже за час власного запуску, а не за час останнього знімка), штамп — лише якщо `merged_at` у цьому вікні (поза ним `cleanup-old-snapshots` лишає тільки останній знімок паба, і свідок злиття міг зникнути); кілька броварень — копія на пару без піна й штампа, з confidence не вище 0.99, щоб пізніше злиття її сироти лише переспрямувало лінк, а не штампувало (`mergeIntoCanonical` штампує лише `WHERE confidence >= 1.0`); жодної — видалення: ні показ, ні #486 такий лінк не читають. Dry-run справжнім кодом на байтовій копії прод-БД: 5411 → 1685 лінків, 7 пінів, 20 штампів; показ 1692 кранів і множина «на крані» (932) без змін |
| 34 | `legacy_card_repairs` (#696) — сталий журнал доказового ремонту старих карток; без бекфілу, один запис на видалену сироту (§3.6.2) |
| 35 | `legacy_orphan_dispositions` (#695) — історія операторських рішень про неактивні старі сироти; без бекфілу, часткові унікальні індекси на активний `beer_id` і точний ключ картки (§3.6.3) |
...
нічого, ні по батчу, ні по файлу в цілому. Причина — не недогляд, а відмова від
самої спроби: довести з рядків файлу, що він є повною, актуальною й унікальною
історією користувача, не вдалося жодним разом. Перша версія заявляла `[MIN, MAX]`
на довірі; гейт проти `countCheckins` рахував рядки з будь-якого джерела, тож
рядки, вже злиті живою синхронізацією, підмінювали власне рядки файлу; гейт проти
лічильника рядків самого файлу пробивали дублікати id (накручують кількість без
розширення діапазону) і застаріла вигрузка з зайвим рядком, якого на Untappd уже
нема (накручує кількість повз саме ту діру, яку мала виявити жива синхронізація).
Три різні обходи одного й того самого припущення — ознака, що припущення хибне
як таке, а не що чергову діру можна залатати черговим гейтом.

Заявку про покриття робить лише перший живий обхід розширення (той самий
механізм, що замінив сид міграції 29) — і саме тому він же знаходить будь-що, що
застарілий рядок з файлу міг би замаскувати. Ціна цього рішення явна: ні міграція
29, ні `/import` нічого не заявляють, тож перший живий обхід кожного користувача
проходить всю його історію заново один раз.
...
```json
{ "beers": [{ "brewery": "string", "name": "string", "abv": 0.0, "bid": 12345, "brand": "string" }] }
```
Масив від 1 до 200 елементів; `abv` — опційний. `bid` і `brand` (#633) — теж опційні й адитивні:
`bid` — Untappd-id, який крамниця публікує на сторінці товару (ціле додатне; невалідне → `400`),
`brand` — бренд із тієї ж сторінки, проти якого цей bid перевіряється. Обидва поля живуть **лише**
в тілі `/match`; спільна схема картки (`src/api/match-input.ts`), яку використовує MCP-тула
`match_beers`, їх не приймає — в агента немає сторінки крамниці.
  ...
  "results": [
    {
      ...
      "matched_beer": { "id": 1, "name": "...", "brewery": "...", "rating_global": 4.1, "untappd_id": 12345 },
      "is_drunk": false,
...
(ймовірно випите, без певності); у розширенні дає бейдж «пив» із маркером непевності
(§6.1) і кліком на Untappd. **`source` і `searched` розширення читає з цієї ж відповіді**
(#648): `source: 'fuzzy'` — єдине джерело непевності, а `searched: false` відрізняє «не
знайшли» від «не шукали». Обидва поля надсилалися завжди; до #648 клієнт їх не оголошував. `user_rating` — особиста оцінка з `checkins` (або `null`); лише для
exact-матчів. Серверна помилка → `500 { error: "internal" }`.

`source` (`exact`/`fuzzy`/`null`) — як саме досягнуто рядок каталогу; `null` — збігу
нема взагалі. `searched: false` — бюджетований повнокаталожний fuzzy-фолбек
(`FULL_FALLBACK_BUDGET`, 20 позицій на запит) для цієї позиції **не запускався**, тож
`matched_beer: null` тут означає «не шукали», а не «нема в каталозі»; exact-стадії при
цьому відпрацювали. Обидва поля адитивні — наявні клієнти розширення їх ігнорують.

**Відповідь за опублікованим bid (#633).** Коли картка несе `bid`, чий рядок є в цьому ж знімку
каталогу (`beers.untappd_id`), він перевіряється **до** аліасів (§3.6.1) і матчера. Доказ броварні —
`brand`; для заглушки Flasker `Імпортне пиво` (розділ вітрини, а не броварня) — `brewery` картки;
**без `brand` bid не відповідає взагалі** (броварня картки сама по собі не свідок: у Flasker вона
виводиться з назви товару й буває неправильною, #650). Далі:
1. броварня збігається (`bidBreweryAgrees`, той самий предикат, що в `resolveByBid`) → `source: "exact"`
   на рядку bid, з `is_drunk`/`user_rating`; аліас і матчер не запускаються, бюджет фолбеку не витрачається;
2. не збігається, але аліас чи матчер самі привели на **той самий** рядок → `exact` на ньому (два незалежні свідки);
3. не збігається, а картку врятував **аліас #614** → лишається рядок аліасу: доведена ідентичність картки
   важить більше за bid, який гейт відхилив. Розбіжність не ховається — `untappd_id` відповіді ≠ `bid`,
   тож клієнт веде картку в репарацію `/enrich/*` (#384);
4. інакше → рядок bid як `source: "fuzzy"` з `is_drunk: false`, `user_rating: null` і `drunk_uncertain`
   за звичайним правилом; `searched` несе відповідь матчера, тож відмова бюджету не ховається.
...
(напр. рев'юеру Chrome Web Store без токена бота) одразу показувати бейджі знайденого
пива й ненайденого (§6.1) — без особистих, бо `drunkSet` порожній. `/enrich/*`
та `/checkins/*` лишаються тільки-за-токеном.

**Name-keys матчинг (order/collab-aware, #117).** Збіг назв — це перетин **множин
канонічних ключів** (`nameKeys`, `matcher.ts`): назва ріжеться на `NAME_COLLAB_SEP`-сторони
(`/`, ` x `), тоді як для броварень `BREWERY_COLLAB_SEP` додатково підтримує `&` (з пробілами чи без [#589]) та ` + ` (#401) —
це запобігає розбиттю дескрипторів смаку в назві пива на кшталт `Salt&Vinegar` (#671). Кожна сторона нормалізується, зрізається продубльована провідна пивоварня,
...
Коли strict-only near-name стадія має approximate top-кандидатів, але чинний score/popularity resolver
не може вибрати один `bid` і вже мав би завершити lookup як `not_found`, `lookupBeer` має
strict collab-token boundary rescue (#613) для збереженого магазином учасника колаборації,
склеєного в назві пива (`MultiQlti` ↔ `Multi Qlti`). Це не загальна нечутливість до меж
слів: один повний нормалізований
collab-part мусить бути токеном вхідної назви й точно дорівнювати конкатенації двох або
...
- **правило:** жорстке число входу, не покрите числом, градусом чи м'яким числом кандидата → `different`
  ...
  має власне непокрите м'яке число чи версію (`Trappistes Rochefort 10` → `6`) або кандидат взагалі не містить літер поза числами (#663: суто цифрові кандидати `21`, `15`); градуси й м'які числа самі не
...
**`lookupBeer`** прибирає кандидатів `different` до будь-якої стадії, а `number-fallback` — коли є кращий рівень того
самого ряду (та сама назва без цифр у тієї самої броварні кандидата)
(нерозфільтрований список лишається доказом у `not_found.candidates`). **`/enrich/*`** обирає рядок пари лише серед
`same` > `year-fallback` (найстаріший у рівні): у цей рядок пишеться bid картки, тож `number-fallback` став би
ідентичністю чужого номера — картка без номера отримує свою сироту. **Сироти:** `ensureOrphan` порівнює рівних (тексти крана) — сумісні, якщо
...
**Веб-фолбек** (#139) відхиляє кандидата `different` як `reject:digits` до гейту назви; **`dedupeBreweryAliases`**
не зливає сироту з канонічним рядком, якщо цифри `different`. Поза правилом свідомо лишаються: групування
неприв'язаних кранів у `/newbeers` (лише показ) і `cleanup-polluted-ontap`, який зливає за результатом матчера
(`number-fallback` дає там `exact`, як і при інжесті).

**Назви кранів лише зі стилю (#663).** Коли назва крана складається виключно зі стильових слів, градусів,
спец-позначок чи шуму (`Pils 12°`, `Stout`, `IPA 13,5°`, `LAGER 10.5°`), `normalizeName` згортає її в порожній
рядок `''`. Усі пива броварні в базі при цьому розділяють нормалізовану пару `(brewery, '')`, що створювало ризик
хибного лінкування на випадкове пиво тієї ж броварні. Захист побудовано на трьох рівнях:
- **Ідентичність стилю (`styleNameIdentity`).** `baseNormalize(stripSearchNoise(rawName))` з обов'язковим
  відрізанням відлуння пивоварні (`stripBreweryFromName`).
- **Гейт точного матчингу (`matchPrepared`).** Якщо `normalized_name === ''`, кандидати точного збігу
  допускаються лише за умови абсолютної рівності `styleNameIdentity(tap.name, tap.brewery) === styleNameIdentity(cand.name, cand.brewery)`
  **та** сумісності за ABV у межах `ABV_TOLERANCE` (якщо обидва мають ABV). Це зберігає легітимні флагманські лінки
  (`Zakładowy Pils`, `Trzech Kumpli Weizen`, `Zwierzyniec Pils`), блокуючи випадкові збіги на чужі стилі броварні.
- **Повна заборона fuzzy-матчингу.** Якщо `normalizeName(name) === ''`, перехід у fuzzy-стадію категорично
  заборонений (`return null` без виклику `Searcher`).
- **Ізоляція сиріт (`ensureOrphan`, `resolvableOrphan`, `ensureBeerRow`).** Сироти та рядки з порожнім
  `normalized_name` вважаються сумісними лише тоді, коли їхні непорожні `styleNameIdentity` збігаються
  (а при порожній ідентичності стилю, напр. суто градусні назви `10°` / `12°`, вимагається точний збіг назви)
  **та** їхні ABV сумісні у межах `ABV_TOLERANCE` (якщо обидва відомі). Картка крамниці чи кран одного стилю
  (`Stout`) чи іншої міцності (`Lager 7.5%` проти `4.0%`) ніколи не бере рядок чи сироту іншого стилю або іншого ABV.
...
`drunk_uncertain: true` (ймовірно випите, без певності); `is_drunk` і `user_rating`
залишаються `false`/`null`. Звідси інваріант, на який спирається розширення:
`drunk_uncertain` ніколи не приходить без `source: 'fuzzy'`.
...
наближений fuzzy (≥0.85, але <1.0). Якщо вхідна броварня порожня (#405), exact name-key перетин
обчислюється як `nameKeys(name, r.brewery_name)` (зрізається броварня кандидата з повної назви),
а для однотокенних назв exact-перевірка приймає кандидата, якщо `stripBreweryFromName` зрізав
хоча б один токен і залишок точно дорівнює назві пива кандидата. Strict-шлях незмінний. (`/match`-каталог поки не зачеплено.)
...
`<вхідна brewery/brand форма> + <повна вхідна назва>`. Порівняння проводиться як над базовою
формою, так і після зрізу провідних дескрипторів пивоварні (`BREWERY_NOISE`, #501), а вхідні ідентичності додатково поповнюються
багатотокенними `COLLAB_SEP`-сторонами назви для випадків колабів у назві або порожньої броварні (#401).
Голий alias назви пива недостатній: `Magic Road Dżemer` доводить конкретну колаборацію, але не створює
глобального alias `Magic Road` ↔ `Sadyba`. `brewery_alias` утворює окремий native-alias pool, не поповнює
курований граф і не успадковує Algolia rank-1: exact/near-name кандидати мають дати один
унікальний результат; при множині правило залежить від того, **яким** доказом побудований
пул. На **точному** ключі (identity-аліаси, native name-keys, brand-remainder) виграє рівно
один кандидат у `ABV_TOLERANCE` — ABV там розділяє вінтажі одного пива. На **наближеному**
near-name пулі (#487) ABV більше не обирає: рішення ухвалює домінування за `rating_count`, а
ABV лише накладає вето. Виняток: стара поведінка (вибір рівно одним кандидатом у
`ABV_TOLERANCE`, як на точному ключі) лишається за двох незалежних умов — **один кандидат** у
топ-скорі (вибирати нема між чим, тож `FLAGSHIP_MIN_RATINGS` до нього взагалі не застосовується)
або **жоден кандидат не несе `rating_count`** (транспорт без popularity-сигналу — legacy
`htmlSearch`-relay, web-фолбек — не можна лишати ще й без ABV).
Термінальна флагман-стадія (#487, нижче) такого фолбека **не має** — вона нова, попередньої
ABV-поведінки в неї немає, тож без `rating_count` вона просто не спрацьовує. Canonical/curated
strict semantics незмінні.
...
(вхідна броварня взаємно покриває всі токени назви пива кандидата, а вхідна назва взаємно покриває броварню
кандидата через `swappedBrandNameScore`), кандидат оцінюється у `brandPool` і приймається лише за
наявності суворого підтвердження ABV (`abv != null` та `Math.abs(r.abv - abv) <= ABV_TOLERANCE`) для
єдиного сумісного кандидата.

**Флагман на голому бренді (#487).** Термінальна стадія `lookupBeer`, після всіх інших — включно з
точним typo-rescue назви, який інакше вона б випереджала: спрацьовує
...
Стадія термінальна за побудовою — вона виконується останньою з усіх, typo-rescue включно, — тож не
може змінити жоден наявний матч, лише перетворити сироту на матч (`Blue Moon` → `Belgian White`, `Pilsner Urquell`, `Primátor Weizen`). Порогові значення
виміряні: правильні флагмани лежать на 5.89×–326×, монетки — на 1.09×–2.45×, між ними порожньо.

#### `POST /mcp` — MCP-сервер для агентів

Той самий матчинг, що й `/match`, але як MCP-тула для AI-агентів (Claude Code,
Codex CLI). Транспорт — streamable HTTP на `@modelcontextprotocol/sdk`
(`WebStandardStreamableHTTPServerTransport`), **stateless**
(`sessionIdGenerator: undefined`) і `enableJsonResponse: true`. Чому саме так:
сесія живе в пам'яті процесу, а процес перезапускається на кожному деплої — клієнт,
що тримає session id, діставав би `404` на наступному виклику замість прозорого
реконекту; а другий довгоживучий шлях крізь cloudflared — це рівно #124.

Обробляється **тільки POST** (`app.post`, не `app.all`). Будь-який інший метод
(`GET`, `DELETE`) → `405 {"error":"method_not_allowed"}`; SSE-стрім не
відкривається ніколи. Причина не косметична: `enableJsonResponse` керує лише
відповідями на POST і GET не гейтить, тож `app.all` пустив би GET (який MCP-клієнти
пробують рутинно) у SSE-гілку SDK — довгоживучий стрім із увімкненим за замовчуванням
keep-alive-таймером, якому тут ніхто не робить `close()`. Автентифікація стоїть на
**шляху**, а не на методі, тож запит без токена дістає `401` раніше, ніж `405`.

Авторизація **обов'язкова** (`authMiddleware`, той самий, що на `/enrich/*`),
анонімного шляху немає — на відміну від `/match` (#245). Невалідний або відсутній
токен → `401 {"error":"unauthorized"}`. Причина не безпекова: анонімна відповідь має `is_drunk: false`
на кожній позиції, і агент переказує це як «ти нічого з цього не пив». У розширенні
⚪ візуально відрізняється від ⭐, у переказаному тексті ця різниця зникає повністю.
Ліміт тіла — `MATCH_BODY_LIMIT_BYTES` (**256 KiB**, той самий, що в `/match`),
змонтований **перед** автентифікацією: завелике тіло дає
`413 {"error":"payload_too_large"}` ще до будь-якого lookup'у токена.

На рукостисканні (`initialize`) сервер представляється іменем `warsaw-beer` і версією
`1.0.0` (`MCP_SERVER_NAME`/`MCP_SERVER_VERSION` у `src/api/mcp/server.ts`) — саме це ім'я
клієнт показує у своєму списку MCP-серверів.

Рівно одна тула — `match_beers`. Вхід: `beers: [{ brewery, name, abv? }]`, від 1 до
200 елементів; `brewery` і `name` обов'язкові обидва (спільна zod-схема з `/match`,
`src/api/match-input.ts` — один опис лімітів на два call-site'и).

**Вихід** (`structuredContent`; плюс текстове дзеркало того самого для клієнтів, які
показують лише текст):
```json
{
  "profile": {
    "checkins_known": 0,
  ...
  "results": [
    {
      ...
      "beer": {
        ...
        "brewery": "...",
        "rating_global": 4.1,
        "untappd_url": "https://untappd.com/beer/12345"
      },
      "your_rating": null
    }
  ]
...
```
`profile` — доказ, на якому стоїть кожна персональна заява у відповіді: скільки
чекінів і скільки `untappd_had` відомо, коли був останній чекін і чи порожній
drunk-set. `drunk_set_empty` рахується **не** з `checkins_known === 0`: чекін із
`beer_id IS NULL` (так і не зматчений на каталог) додає до `checkins_known`, але в
drunk-set не дає нічого, тож ці двоє можуть розходитись. `confidence` —
`exact`/`fuzzy`/`null` (нема збігу); `your_rating` віддається лише для
exact-матчів, як і в `/match`. `beer.untappd_url` будується з `beers.untappd_id`,
ніколи з `match_links.untappd_beer_id` (§5.2).

**Шість статусів** — і що саме кожен стверджує:

| Статус | Заява |
|--------|-------|
| `drunk` | exact-збіг, і пиво в drunk-set (`checkins ∪ untappd_had`) — випите напевно |
| `probably_drunk` | fuzzy-збіг, і пиво в drunk-set — ймовірно випите, без певності; особиста оцінка тут не віддається |
| `not_drunk` | збіг є, пива в drunk-set немає — **і drunk-set непорожній** |
| `unknown` | збіг є, але про питі цим користувачем пива не відомо **нічого** (drunk-set порожній) — це не «не пив», а «нема звідки знати» |
| `not_in_catalog` | позицію пройшли всі стадії, які до неї застосовні, і збігу нема |
| `not_searched` | бюджетований повнокаталожний fuzzy-пошук для цієї позиції **не запускався** |

Два розрізнення тут несучі. **`not_in_catalog` ≠ `not_searched`**: повнокаталожний
fuzzy-фолбек має бюджет `FULL_FALLBACK_BUDGET` (20 позицій на запит), і позиції за
бюджетом лишаються непереглянутими. У повний каталог позиція падає лише тоді, коли
жодна броварня каталогу не збіглася з вхідною (інакше fuzzy йде по бакету броварні й
бюджету не потребує). `not_searched` **не** означає «ми на це не дивилися взагалі»:
exact-стадії відпрацювали й промахнулись — пропущено саме повнокаталожний fuzzy, тож
промах тут не є свідченням, що пива в каталозі нема. Агент, який зіллє ці два
статуси, ствердить факт про дані, яких ніхто не переглядав. Друге розрізнення —
**`not_drunk` → `unknown` при порожньому drunk-set**: «ти це не пив» — заява про
людину, і порожній drunk-set її не підтримує; каталожна половина відповіді при цьому
не міняється.

Помилка виконання тули приходить як **HTTP 200 із JSON-RPC `result`, усередині якого
`isError: true`** — а не як JSON-RPC `error`: SDK ловить кинуту в хендлері
`tools/call` помилку й повертає її звичайною відповіддю (закріплено тестом у
`src/api/routes/mcp.test.ts`). **Той самий шлях бере й невалідний вхід** — порожній
`beers`, більш ніж 200 елементів, відсутнє поле: SDK валідує аргументи всередині того
самого `try` і кидає `McpError(InvalidParams)`, який не є єдиним кодом, що
перекидається протоколом, тож клієнт отримує `200` + `result.isError: true` з текстом
`MCP error -32602: Input validation error…`. Тобто `/mcp`, на відміну від `/match`,
**не** відповідає `400` на невалідне тіло — шукати такий код у логах марно (перевірено
живою пробою на всіх трьох формах невалідного входу). Виняток один — перевищення ліміту
тіла: його ловить `payloadBodyLimit` ще до SDK, тож воно приходить як HTTP-413, поза
JSON-RPC-формою.

Облік для дайджесту (`api_usage.mcp_requests`/`mcp_beers`, §3.16) інкрементується **в
хендлері тули**, а не в route: через той самий route ходять `initialize` і
`tools/list`, а рукостискання — не матчинг.

Клієнти: Claude Code (`claude mcp add --transport http … --header "Authorization:
Bearer <token>"`) і Codex CLI (`bearer_token_env_var`). Токен — той самий, що й для
браузерного розширення (`/extension`); окремого токена для MCP немає. claude.ai
(веб/десктоп) **не** підтримується: custom connectors там поля для статичного
заголовка не мають узагалі, лише OAuth Client ID/Secret. Інструкція для користувача —
`docs/mcp-uk.md` (публікується на GitHub Pages).
...
провенансу канонічного рядка (вето `not_a_beer` і бекоф діють), а в `/enrich/result` прийнятий bid
записується на сироту картки (`ensureOrphan`: наявна сирота пари або нова; злінкований рядок пари — ніколи), і аліас переходить туди разом із доказом (`ensureOrphan` → `applyLookupOutcome`: злиття у власника bid або новий
...
**Zero-hit ретрай дескрипторів стилів та пакування (#353).** Коли пошуковий запит
повертає **нуль** кандидатів за всіма частинами пивоварні та щаблями драбини (справжнє
занулення через переобтяження токенами в Algolia, а не відхилення матчером), `lookupBeer`
виконує послідовні одноразові fallback-ретраї з очищеною назвою:
1. Спочатку перевіряється `#271` head-retry (`headBeforeTail` відрізає комовий або `#N` хвіст,
   ігноруючи десяткові коми на кшталт `12,5°`).
2. Якщо пошук все одно дав 0 кандидатів, спрацьовує `stripDescriptorAndPackaging(name)`:
   зрізає кінцеві токени тари й об'єму (`can`, `bottle`, `pack`, `4-pack`, `keg`, `473ml`,
   `500ml`, `0.5l`) та кінцеві стильові/фільтраційні дескриптори (`West Coast IPA`,
   `Foreign Extra Stout`, `Tomato Gose`, `niepasteryzowane`, `Pale Ale`, `Jasny lager`,
   `Malt Beer KVAS`, `hard seltzer` тощо), за умови наявності брендових токенів перед ними.
3. Якщо скорочена назва непорожня й відрізняється від очищеної вхідної, виконується повторний
   запит `lookupBeer` з прапорцем `descriptorRetried`.
4. **Гейти безпеки на шляху ретраю:** кандидат, знайдений через дескрипторний ретрай,
   підлягає двом обов'язковим перевіркам:
   - **Alcohol-Class гейт:** безалкогольний вхід (`abv <= 0.7` або наявність слів
     `bezalkoholowe`, `non-alcoholic`, `alkofrei`, `alkoholfrei`, `nealko`, `0.0`, `zero`)
     **ніколи** не матчиться з алкогольним кандидатом (`abv >= 2.0`) — це запобігає хибному
     збігу з алкогольним близнюком (#33783). Навпаки, вхід з `abv >= 2.0` без безалкогольних
     дескрипторів не матчиться з безалкогольним кандидатом.
   - **ABV-толеранс гейт:** коли обидва ABV відомі, різниця не може перевищувати `ABV_TOLERANCE`
     (0.3%) — це запобігає підміні схожого сорту іншої міцності (#33517).
   У разі провалу будь-якого з гейтів кандидат відхиляється, і пошук термінально повертає `not_found`.
...
її в `1` — див. §3.14), **але не гарантовано `false`**: міграція 29 не чистить історичні
рядки, і рядок, защипнутий у `1` старим `advanceSyncState` ще до #587, лишається `1`
назавжди (сьогодні всі живі рядки тримають `0` — випадковість стану БД, не гарантія
контракту); `profileTotal` тут завжди `null` (маршрут
не читає `checkin_sync_state.profile_total` — pre-existing, залишено: клієнт перезаписує це
поле відповіддю `POST` тієї ж секунди, ціна — лише перший рендер попапу без знаменника).
...
чекін мав би повернутися. Це трактується як зламана Untappd-сесія або блок:
`422 { error: "no_session" }`, курсор не просувається. Порожня відповідь на курсорі `≤
oldestCheckinId` — законне дно фіду **або** та сама мертва сесія (за вмістом вони
нерозрізненні); тут обхід просто завершується (`nextCursor: null`), без жодного запису
про дно. **Відомий хибний позитив, свідомо прийнятий:** користувач, що видалив на Untappd
самі ті чекіни, які ми ще тримаємо, отримає постійний `no_session`, хоча сесія жива —
альтернатива («порожня відповідь = успіх») це і є те, що спричинило #587 в іншу двері.

**Суперечлива пара курсор/сторінка → `400 bad_cursor`.** Якщо найстаріший елемент
сторінки ВИЩИЙ за надісланий курсор — це не той фрагмент, який просили (типово: `more_feed`
307-редіректить на сторінку профілю). Записати її діапазон означало б ствердити покриття з
чужої сторінки, тож пара відхиляється до будь-якого запису. **Рев'ю PR #592 (P2): та сама
перевірка йде й для верхнього кінця** — сторінка з найновішим елементом ВИЩИМ за курсор так
само не той фрагмент (легітимний фрагмент повертає лише елементи строго нижче свого
курсора), навіть якщо найстаріший елемент сам по собі не вищий за курсор; відхиляється тим
самим `400 bad_cursor`, тим самим вартовим до транзакції.

**`nextCursor` — куди йти далі, рахує сервер.** Три умови дають `null`, і жодна не є
твердженням про дно стрічки — усі про «роботи більше немає»:

1. порожня сторінка на курсорі `≤ oldestCheckinId` (дно або мертва сесія — нерозрізненно, §3.15);
2. лічильники зійшлися (`serverCount >= profile_total`) **і** покриття користувача — один
   суцільний діапазон. Друга умова обов'язкова: сама лише рівність лічильників не досить, бо
   `profile_total` може ЗМЕНШИТИСЯ (користувач видалив чекін на Untappd, наш рядок лишився) —
   тоді числа «зійдуться» над дірою, яку жоден майбутній прогін уже не спробує закрити;
3. (вартовий, не бізнес-умова) наступний курсор не опустився строго нижче надісланого —
   сервер примусово ставить `null`. Це єдиний захист від зациклення: клієнт (з Task 5)
   власного вартового монотонності більше не має, і без цього рядка спотворена сторінка
   означала б спін на 200 запитів до кепу, а не чисту зупинку.

Інакше `nextCursor` — це `from_id` діапазону покриття, у який потрапив низ поточної
сторінки (щойно злитого діапазону чи вже наявного) — стрибок під усю відому територію
одним кроком, а не по сторінці за раз.
...
(`offset` = курсор `nextCursor`), з заголовком `X-Requested-With:
XMLHttpRequest` (без нього Untappd 307-редіректить на `/home`). ⚠️ `?max_id=` на сторінці профілю
**ігнорується** (завжди віддає найновішу сторінку) — НЕ використовувати.

**Обхід (клієнт) — один цикл, без фаз.** З #587 дві фази (top-up над відомим, deep-extend від
збереженого курсора) зникли: клієнт просто йде за `nextCursor`, доки сервер не поверне `null`
або не спрацює жорсткий cap (~200 сторінок/прогін) — обидві умови трактуються однаково,
різниця лише в кінцевому стані попапу (`done` проти `capped`). Для повністю покритого
користувача сталий режим — **один** запит на клік (сторінка профілю; лічильники зійшлися →
`nextCursor: null` одразу). Для користувача з дірою обхід іде, доки не закриє її, і може
зайняти час до кепу — індикація прогресу і «Stop» уже існують (#520). Поки прогін активний,
primary-кнопка стає «Stop»; повідомлення `checkin-sync:stop` скасовує активний feed-запит,
submit сторінки до backend або паузу між сторінками й завершує прогін зі станом `cancelled`.
Після cap кнопка явно показує «Continue — N left» (або «Continue sync», якщо `profileTotal`
невідомий), але наступний bounded-прогін запускається лише кліком користувача. Окремий
термінальний стан `no_session` (мапиться з `422`) веде власний текст у попапі — «сесія
протермінувалась», а не мовчазне «синхронізовано».
...
| `dailyStatus` | `*/15 * * * *` | health-дайджест адміну. UTC-тік; джоба сама шле раз на варшавську добу у вікні `[09:00, 12:00)` Europe/Warsaw, ідемпотентно за `job_state.daily_status_last_sent` (лише якщо є `ADMIN_TELEGRAM_ID`). Раніше `0 9 * * * {tz}` — timezone-тік node-cron виявився ненадійним. Дайджест включає секцію **Enrich**: `enrichMatched24h` зматчених orphan'ів, `enrichFailures24h` провалів, індикатор здоров'я Untappd-пошуку (`untappdSearchHealthy` = останній canary ok І breaker не відкритий). Рядок «Каталог» також несе `orphansRelayQueue` («N у relay-черзі») — orphan'и, які зараз не на крані, за винятком `not_a_beer`/`retired`, тобто розмір черги relay-дренажу (#368/#486). Рядок **«Рейтинги»**: зматчених пив без рейтингу і скільки злінкованих пив звірено з Untappd за 30 днів (`rating_checked_at`, #616) — друге число показує, чи законні `NULL` (менш ніж 10 оцінок) звірені, а не забуті. Рядок «Розширення /match (вчора)»: усього запитів, з них анонімних, і сума пив за **останню повну варшавську добу** (з таблиці `api_usage`, §3.16). Одразу під ним іде рядок **«MCP /match (вчора)»** — запитів і пив за ту саму добу з колонок `mcp_*` (§3.16). Анонімного розрізнення в ньому нема: `/mcp` не має анонімного шляху. Нулі показуються, якщо трафіку не було. Одразу під рядком тріажу — рядок **«Насичені»** (#431): `#<issue> (<рядків>)` для issue, що набрали ≥ `SATURATION_ALERT_ROWS` рядків після створення, за спаданням, топ-5 плюс «усього N». Рядок відсутній не лише коли насичених немає, а й коли сьогодні взагалі не було батчу для тріажу (порожня вибірка orphan'ів) — відсутність означає «немає насичених АБО не було батчу сьогодні», і достовірний стан читається з мітки `saturated` на GitHub, а не з відсутності цього рядка. Це водночас звіт і черга робіт: перший у списку — наступний кандидат на фікс. Рядок **«Печатки»** (#377, одразу після рядка «Насичені» — спершу сьогоднішній прогін, потім накопичений стан): `N unidentifiable (M переспостережено)` — `M` рахує рядки, у яких `beers.untappd_lookup_at > enrich_failures.reviewed_at`, тобто чи крон реально до них доходить; **`M = 0` означає, що механізм досяжності мертвий**, а високе `M` при незмінному `N` — що він крутиться намарно (порахувати *зняті* печатки неможливо: авто-розпечатувач занулює і `review_class`, і `reviewed_at`). Далі `P not_a_beer (+Q/7д)` — борг фільтра інжесту, і `R спростованих retire` — рядки з `retired_at`, чиє пиво досі orphan, тобто твердження «фікс це вирішив», спростоване самим існуванням рядка |
...
останньому снапшоті хоча б одного паба (`match_links → taps → tap_snapshots`, лінк з'єднується з краном за парою
броварні й назви, #632). **Relay-пул**
(`listRelayLookupCandidates`) — orphan'и, які зараз не на крані: їх намінтив
`/enrich/candidates` (`ensureBeerRow` біжить по кожній картці сторінки крамниці), а лінки
пише лише on-tap ingest, тож до #486 on-tap join виключав їх **структурно**, а не
через схід із кранів (на 2026-08-08 це 846 із 1427 orphan'ів, 532 з них не шукані жодного
разу; сьогоднішній склад пулу ширший — нижче).
...
`not_a_beer`, не `retired`, без активного рішення #695 і не під замком, потрапляє **рівно в один** пул: обидва предикати
...
`review_class = 'not_a_beer'`, `retired_at IS NOT NULL` і активне рішення #695 та однаково фільтруються backoff'ом.
...
Лічильник називається «у relay-черзі», бо саме її й міряє. «Рівно в один пул» вище —
властивість самої партиції: після #486 немає orphan'а поза `not_a_beer`/`retired`/замком, який
не влучив би в жоден з двох пулів. Замок (`lockedRowPredicate`, «Замок за фіксом», #421) —
окреме, свідоме виключення поверх партиції: доки issue відкритий, рядок лишається поза обома
активними запитами (`listLookupCandidates`, `listRelayLookupCandidates`), хоч і входить у
партицію. `orphansRelayQueue` цей фільтр не застосовує — він рахує чергу, а не слот, готовий до
запиту просто зараз, — тож замкнені рядки в лічильнику лишаються видимими.
...
ставить `rating_checked_at`; bid, якого Algolia не знає (`null` на його позиції), отримує лише бекоф.
Запис на позиції, що не розібрався або належить іншому bid, нічого не доводить: цей bid пропускається без
записів, решта пачки звіряється. Відповідь без `results` або з іншою кількістю результатів — транзієнт:
нічого не пишеться. Провенанс гідратор не змінює. `refreshAllUntappd` на рядку, знайденому
за bid, розрізняє три стани картки: блок «Global Rating» з числом — перезапис і штамп; «N/A» — `NULL` і
штамп; блоку немає — рейтинг і штамп не чіпаються. Підпис «Global Rating» без читабельного `data-rating`
(і не «(N/A)») нічого не доводить і рахується як відсутній блок. Штамп профілю скидає бекоф гідратора:
сторінка довела, що bid живий. Там само провенанс лінка посилюється до `checkin`
...
- **Ontap tap-exclusion gate.** `refreshOntap` ПОВИНЕН відкидати **до** створення
  ...
  (wine/prosecco/frizzante/spritz/cocktails/nalewka/szprycer, brewery_ref-сміття
  ...
  `wino`/`merlot`. Cider, kvass/`Kwas chlebowy`/`квас`, mead/melomel і kombucha лишаються eligible.
...
- **Лінк крана ключується парою точного тексту броварні й назви крана (#632).** Кран однієї броварні ніколи не
  переписує лінк і не скидає штамп злиття крана іншої броварні з тією самою назвою; показ крана й «на крані» (#486)
  з'єднують кран із лінком за цією парою. Виняток — дві пари, чий текст нормалізатор зводить до одного ключа сироти
  (`ensureOrphan`): вони ділять сироту, і її злиття штампує обидві (клас #636).
- **Ідентичність, здобута злиттям, переживає повторну ту саму картку крамниці (#614).** Ключ аліасу — усе, що
  ...
  ідентичність). Аліас доводить лише той bid, під яким записаний, і лише картку, яка і є злитою сиротою, — або картку, чий
  опублікований bid прийнято (доказ узято з її полів).
  Конфлікт доказів для тієї самої картки розв'язує запис (новіший доказ переносить аліас), а не читання;
...
- **Назва лише зі стилю — тільки суворий exact або orphan (#663).** Якщо `normalizeName(name) === ''`
  (назва містила лише стиль, градус чи шум), fuzzy-стадія категорично заборонена, точний збіг
  вимагає повної рівності `styleNameIdentity` і сумісності за ABV, а сироти й рядки крамниці
  ізолюються за стилем.
- **Фільтр токенів не має права лишити назву без ідентичності (#505).** `normalizeName`
  ...
  Правило застосовується симетрично до вхідної назви і до кандидатів пошуку (`lookupBeer`);
  одностороннє застосування — виміряна регресія. Обсяг: `nameIdentity`/`candidateIdentity`
  споживає лише `lookupBeer` (пошук Untappd) — `matchPrepared` і гарячий шлях `/match` цього
  поля не бачать. Резервний шлях свідомо обходить `stripSearchNoise` (на відміну від
  `normalizeName`): пропустивши сирy назву крізь нього теж, `Żywiec 0,0%` згорнувся б назад
  у голий бренд `żywiec` і зняв би шість відмов від хибного лінку безалкогольного пива на
  його ж хмільного тезку — бонус, який цей дизайн собі зараховує.
...
- **`fetch` — з пакета `undici`, не глобальний (#581).** `createHttp` і
  ...
  Стосується обох інстансів `createHttp` — і проксованого, і прямого (скрейп магазинів): фабрика одна на двох.
- **Стратегія ротації exit-IP (#222).** `p.webshare.io` не ротує в межах однієї
  ...
  процес. Виправлення: безкукова клієнт (search-key fetch; до #616 — ще й `refresh-tap-ratings`)
...
- **Рев'ювер друкує власний рахунок.** `callStructured` повертає `usage` разом із
  ...
  останньої перевірки** (перевірено 2026-09-22: `gpt-5.5` = $5 / $0.50 / $30,
  `gpt-5.6-sol` = $4 / $0.40 / $20, `gpt-5.6-terra` = $2 / $0.20 / $12,
  `gpt-5.6-luna` = $0.20 / $0.02 / $1.20 за 1M). Ціна `gpt-5.6-sol` —
  **промоційна, гарантована лише до 2026-11-21**, далі може вернутись до
  $5 / $30, тобто зрівнятися з `gpt-5.5`; коли це станеться, таблиця почне
  **занижувати** рахунок мовчки, тож перевірити її треба не пізніше тієї дати.
  Модель без перевіреної ціни друкує токени **без** доларів, а сума PR
...
- **Контекст:** `scripts/ai-review/context.ts` шле моделі і діф, і повний HEAD-вміст
  змінених файлів (кожного, чий `readFile` не повернув `null` — див. виняток для
  тестових файлів нижче), у порядку спадання churn (додані+видалені рядки), поки не
  ...
  Тіло тестового файлу не потрапляє у зібраний **find-контекст**: у нього йде лише діф,
  знахідки в тестовому файлі лишаються легальними, і сам файл потрапляє у список «Files
  where you see only the diff» (`BODY_EXCLUDE_PATTERNS` і `contextReader`, обидва —
  `scripts/ai-pr-review.ts`, застосовуються обома викликачами `buildReviewContext`:
  `scripts/ai-pr-review.ts` і `scripts/ai-review/replay.ts`). Це не «тіло не шлеться
  ніколи» — `verify.ts` інлайнить повне тіло будь-якого файлу з зауваженням, що пережило
  гейт, тестові файли не виняток. Причина обмеження саме find-контексту — вимір
  2026-09-22: тіла тестів займали 41 % зібраного контексту, а що порядок спадання churn
  ставить тест поперед його ж сорсу, то при впертому бюджеті витіснявся саме
  **вихідний** код (на #670 — 31 файл у diff-only, на #669 контекст містив 168k символів
  ...
  на збірку find-контексту: гейт і verify читають справжній файл, інакше кожне
  зауваження в тесті стало б `quote_not_found`.
...
- **Моделі:** `AI_REVIEW_MODEL` (find) дефолтиться на **`gpt-5.6-sol`**,
  `AI_REVIEW_VERIFY_MODEL` (verify) — на **`gpt-5.5`**. Обидва вибрані виміром
  replay, ніколи вподобанням; протокол і всі числа —
  `docs/ai-review-model-evaluation.md`. Базово (2026-07-28) `gpt-5.5`-find дав
  0 фабрикацій на precision-сеті проти 5 із 10 у `gpt-5.4-mini`. Заміна find на
  `gpt-5.6-sol` (2026-09-22) — **строге покращення**, тому ставки на якість не
  несе: дешевша і по входу ($4 проти $5 за 1M), і по виходу ($20 проти $30), а
  на пробі покриття дає 5 із 5 відомих дефектів плюс кілька понад, 8 і 6
  підтверджених за прогін проти 5/5/3 у `gpt-5.5`. Дешевші рівні зміряні й
  відхилені: `gpt-5.6-terra` бере 3–4 з 5, `gpt-5.6-luna` — лише 2 і **жодного
  разу** за три прогони не піднімає гард нового issue. Verify лишається на
  `gpt-5.5`: це адверсарний суддя, його промпти малі, а частка в рахунку ≈18% —
  грошей у переносі немає, precision втратити є що.
...
- **Offline replay:** `npm run ai-review-replay -- <pr> [base-sha] [--head <sha>]` ганяє
  весь пайплайн (з `base-sha` — саме той інкрементальний зріз, який побачив би CI на
  ...
  **ті самі константи** (`DEFAULT_FIND_MODEL`/`DEFAULT_VERIFY_MODEL` з
  ...
  відсутньому об'єкті. `--head <sha>` перекриває, який коміт вважати HEAD PR-а —
  для recall-проби: заміряти конфігурацію проти голови, яку бачив живий рев'ю
  (з її блоку стану `<!-- ai-pr-review-state {"head":…} -->`), а не проти
  змердженої голови, де знахідки живого рев'ю вже пофікшені й тому будь-яка
  конфігурація виглядає чистою.
- **Розмічений корпус для `verify`:** `npm run verify-corpus -- --model <m>
  [--draws N] [--only <id-prefix>]` ганяє кандидата на модель-суддю проти
  розміченого корпусу тверджень із перевіреним людиною вердиктом — не проти
  іншого судді, бо згода двох суддів міряє схожість, а не правильність. Корпус
  лежить у `scripts/ai-review/verify-corpus.json` поруч зі своєю прогонялкою;
  кожен запис несе `sha`/`file`/`matchedLine`-`matchedEndLine`/`quote` і
  `expected`-вердикт з обґрунтуванням `why_expected`, звіреним із деревом.
  Постить нічого, гроші витрачає лише з переданою моделлю. Прапорець
  `--check` не робить жодного виклику й моделі не бере: він звіряє кожен
  запис корпусу проти пінованого дерева (чи резолвиться `sha`, чи цитата
  збігається з тілом побайтово від `matchedLine`, чи довжина спану відповідає
  довжині цитати), щоб відредагований рядок чи перебита цитата не підмінили
  істину мовчки. Дизайн: `docs/superpowers/specs/2026-09/2026-09-23-verify-corpus-and-runner-design.md`.
...
- **Причина публікується лише перевіреною.** Вердикт, що прив'язує orphan до issue,
  ...
  щодня). Кількість таких рахується там, де вирішується (`causeStripped`), і
  потрапляє в рядок дайджесту як `N неперевірених` — один із трьох непересічних
  «тихих» механізмів для actionable-класу без issue, поряд із `N без цілі`
  (модель не назвала ні issue, ні ключ — `noTarget`) і `N без доказу відсутності`
  (`not_on_untappd` без проби — `guardHits.unprobed_absence`); `not_a_beer` має
  власну окрему частину (`N not_a_beer`) і в жоден із трьох не входить, щоб не
  лічитися двічі (#432). Підстава: рев'ю 2026-07-28 — з ~16 причинних гіпотез
  підтвердились 4, а дві реалізовані погіршили б матчинг (#340, #303).
...
- **Scope — необхідна умова, а не визначення.** Він відповідає лише на питання «чи
  ...
  виражається взагалі.
- **Модель бачить той самий scope, що й гейт (#509).** Тіло issue обрізається до
  `ISSUE_BODY_CAP` (2000 символів) ДО того, як потрапити в промпт, а scope-блок
  рендериться в КІНЕЦЬ тіла — тож великі issue (найбільші власники рядків) обрізали
  власний scope ще до того, як його побачила модель, і відповідали на «чи рядок сюди
  належить» узагалі не бачачи умови. Тепер розпарсений scope кожної відкритої issue
  обчислюється **один раз**, перед викликом LLM, а не після нього, і саме цей
  розпарсений scope рендериться явним рядком `Scope: ...` ПЕРЕД урізаним тілом (фенс
  `triage-scope`, якщо він випадково лишився в тілі, вирізається, щоб лишився рівно
  один запис) — модель і гейт тепер читають структурно той самий предикат.
  `ISSUE_BODY_CAP` не змінюється: після цього він більше нічого змістовного не ріже.
- **Ціль, яка не може прийняти жоден рядок ПОТОЧНОГО БАТЧУ, моделі не пропонується
  (#509, уточнено після рев'ю).** Issue без scope-блоку відмовляє КОЖНОМУ рядку за
  побудовою. Issue з когортою без `where` — інакше: `rowSatisfiesScope` приймає рядок
  за належністю до когорти (`beer_ids.includes`) РАНІШЕ, ніж узагалі гляне на `where`,
  тож когорта-без-`where` відмовляє не «кожному рядку», а лише тим рядкам поточного
  батчу, яких нема в її когорті. `isRoutableTarget` тепер враховує це: ціль
  пропонується моделі, коли `where` непорожній (незалежно від батчу) АБО когорта
  перетинається з `beer_ids` поточного батчу. Виміряно на 26 архівних ранах: на 4 з
  них (#322 з рядком 34642 двічі, #320 з 31816, #320 з 30667) когорта issue без
  `where` перетиналася з батчем того дня — приблизно раз на тиждень, не патологічно,
  але це була розбіжність фільтра й гейта, яку попередня версія фільтра (лише
  `where.length > 0`) ховала від моделі попри те, що гейт прийняв би такий маршрут.
  Фільтр стосується лише промпту: `planTriageActions` і звірка
  мітки `saturated` (`reconcileSaturatedLabels`) і далі отримують ПОВНИЙ список
  відкритих issues без фільтра — гейт мусить лишатися здатним відмовити номер, який
  модель вигадала (а не лише той, що моделі не пропонувався), а звірка `saturated`
  мусить уміти зняти мітку з issue, якої вже немає серед запропонованих моделі цілей.
  Відхилений варіант — фільтрувати той самий список і для гейта: тоді гейт не міг би
  відрізнити «модель обрала легальну, але суперечливу ціль» від «модель вигадала
  номер», а звірка `saturated` втратила б саме ті issues, з яких мітку якраз і треба
  зняти.
- **Три детерміновані гейти вердикту** в `planTriageActions` (чиста функція, без
  LLM); кожен веде лічильник `guardHits`, який публікується в лозі `orphan-triage
  finished` (`outcome.guardHits`) на **кожному** запуску, а не лише коли трапляється
  нестача покриття — `verdict shortfall` лишає собі власний сенс і несе тільки
  `{ covered, batch }` (#432: `covered` рахується ДО того, як гейти відпрацюють, тож
  відхилений рядок і так лічився покритим — гейти могли спрацьовувати мовчки скільки
  завгодно разів). Окремий `warn` («guard anomaly») спрацьовує лише коли ненульові
  `illegal_scope` або `scope_violation` — саме вони означають, що потрібна людина;
  `unprobed_absence` (рутинний за побудовою, #357) його не піднімає:
  ...
  2. Вердикт приліплюється до відкритої issue, лише якщо рядок задовольняє її scope.
     ...
     купу. **Відмова не викидає класифікацію (#509) — крім `not_a_beer`.** Раніше
     відмовлений вердикт ставав `skipped` — рядок лишався нетріаженим і повертався в
     завтрашній батч, де модель робила той самий вибір і отримувала ту саму відмову (на
     проді — 9 днів поспіль той самий рядок на ту саму issue). Для `matcher_bug`/`parser_bug`
     клас тепер **зберігається**, ціль (`issue_number` / `new_issue_key`) знімається, а
     `review_note` стає `off-scope <ціль>: <причина> | <оригінальна нотатка моделі>` (перший
     term, якому рядок суперечить, напр. `candidates_count = 0`; якщо в issue взагалі немає
     блоку scope — літерально `no scope block`, а не «outside the cohort», бо рядок не
     провалив належність до когорти — в issue просто нема scope). `<ціль>` і `<причина>` —
     обидва модельні за походженням без обмеження символів на вході (`new_issue_key`;
     значення `contains`-терма, яке потрапляє в `<причина>` через `describeTerm`) — перед
     склеюванням проходять три кроки, у цьому порядку: (1) будь-яка послідовність
     пробільних символів (переноси рядка, таби, кілька пробілів підряд, а також
     провідні/кінцеві пробіли) стягується в один пробіл і обрізається по краях (`.trim()`)
     — так порожні пробіли не роблять з двох однакових по суті цілей два різні заголовки
     групи в інбоксі; (2) `: ` у цілі й `|` в обох замінюються (на `; ` і `/`
     відповідно), щоб `groupOwnerless` міг розпарсити ціль і причину однозначно, а не
     вгадувати межу регексом; (3) кожне поле окремо обрізається власним лімітом символів
     — ціль до 60 (`MAX_TARGET_CHARS`), причина до 200 (`MAX_REASON_CHARS`) — **до**
     склеювання рядка. Це навмисно окремо від фінального `.slice(0, 500)`: одне лише
     обрізання всього рядка різало б там, де випадково впаде 500-та позиція — для
     достатньо довгої цілі чи причини це середина `": "` або `" | "`, і `groupOwnerless`
     або гублячи ціль, або дописуючи в причину зайвий символ `|`. Обмежуючи ціль і
     причину заздалегідь, структурна частина (`off-scope <ціль>: <причина>`) разом з
     повним роздільником `" | "` завжди складається цілою (60 + 200 + роздільники — це
     менше третини бюджету в 500), тож `.slice(0, 500)` може обрізати вже лише
     оригінальну репліку моделі, що йде третьою. Якщо причина сама по собі досить довга,
     щоб забрати весь бюджет, результат — нотатка без хвоста (`<оригінальна нотатка
     моделі>` відсутня повністю), а не нотатка з половиною роздільника: втратити хвіст
     дешевше, ніж зіпсувати причину, яку людина читає як факт про рядок. Оригінальна
     репліка моделі («shop brand in brewery field» тощо) навмисно
     **не** викидається: для рядка, чия єдина мета після відмови — чекати людину, це єдина
     заява про те, чим дефект насправді є. Рахується окремо від `guardHits.scope_violation`
     (той лічить спрацювання гейта, безумовно, для будь-якого класу) як `quietOffScope`
     (`planTriageActions`) → `outcome.offScope`, і публікується в дайджесті як
     `N без власника (поза scope)`. `not_a_beer`, натомість, узагалі **не** записується
     цим шляхом: гейт лишає `guardHits.scope_violation` інкрементованим (спрацювання
     зафіксоване), але верджикт стає `skipped` замість `quiet` — тобто рядок повертається
     в завтрашній батч так само, як до #509. Причина асиметрії: `not_a_beer` — єдиний клас,
     чий запис **незворотний** (`orphanNotOnTapPredicate` виключає його з обох
     enrich-пулів безумовно, а `listOwnerlessRows` покриває лише `matcher_bug`/`parser_bug`,
     тож тихий запис без issue лишив би рядок поза конвеєром назавжди, без сліду навіть в
     інбоксі). Виміряно реплеєм усіх 28 архівних прогонів: 0 із 62 вердиктів `not_a_beer`
     колись узагалі назвали issue (`matcher_bug` — 302 з 369, `parser_bug` — 15 з 98), тож
     ціна цього винятку — щонайбільше один зайвий LLM-вердикт у день, якого ще жодного разу
     не було.
  3. `not_on_untappd` приймається, лише якщо проба **бігала й повернула порожньо**
     ...
     лишається в пулі збагачення (обидва пули, on-tap і relay, виключають тільки
     `not_a_beer`/`retired_at` — див. «Два пули кандидатів, один бюджет (#368)»).
     Підстава — #377: із 14 слабко доведених вердиктів 7 хибні,
...
- **Триаж-інбокс (#509).** Рядки, чию ціль відмовив scope-гейт вище, не зникають з
  поля зору попри те, що `issue_number` лишається `NULL`: одна стояча GitHub-issue
  з міткою `triage-inbox` перезаписується **цілком** щодня — тіло генерується з
  поточного стану БД, а не накопичується день у день. Рядки в ній згруповані за
  машинно-читаною причиною відмови: за відмовленою ціллю (номером issue чи ключем
  пропонованої нової — `off-scope <ціль>: <причина>`), за «проба не бігала»
  (`no absence evidence:`) або за «причину не підтвердив повторний запит»
  (`unverified:` — це префікс гейта верифікації #358, а не модель; рядки з ним теж
  є власним щоденним виводом цього пайплайна, а не «вільною прозою» з решти). Нотатка, чий
  префікс жодна з цих груп не впізнає, іде в окрему групу «unrecognised», а не
  мовчки в «проба не бігала» — інакше причина відмови в звіті стала б простою
  брехнею. Обмежено 10 групами й 15 рядками на групу; що не влізло — не мовчить, а
  показується як лічильник (і на рівні рядків у групі — «ще N», і на рівні самих
  груп — окремий рядок з кількістю невідображених груп і рядків у них), і заголовок
  рахує **всі** групи, а не лише показані — інакше 11-та й далі групи виглядали б
  вільною прозою моделі, хоча вони так само машинно згруповані й дієві. У заголовку —
  загальна кількість рядків **класів `matcher_bug`/`parser_bug` без issue** за весь
  час, не лише за сьогодні (`unidentifiable`/`not_on_untappd` — тихі класи, вони за
  задумом ніколи не несуть issue, тож у це число не входять). Issue з міткою
  `triage-inbox` **ніколи** не несе мітку `orphan-triage`:
  вона звіт, а не ціль — якби модель бачила її серед відкритих issue з міткою
  `orphan-triage`, рядки могли б замаршрутизуватися в неї так само, як у будь-яку
  іншу, і повторити рівно те, заради усунення чого існує #509. Публікація —
  best-effort і йде **після** всіх записів у БД та GitHub, які запуск уже
  зобов'язаний зробити: збій тут не коштує жодного здобутого вердикту, лише
  лишає вчорашнє тіло інбоксу ще на день. Людина закриває issue, коли розгребла
  список, — це означає «ґрунт», а не «вирішено назавжди»: наступний запуск, не
  побачивши жодної відкритої issue з міткою `triage-inbox`, заводить свіжу. Коли
  ownerless-рядків узагалі немає **і** відкритого інбоксу теж немає — не заводиться
  нічого: інакше щоранку заводилася б порожня issue назавжди, що суперечить самій
  ідеї «закриття = ґрунт». Якщо відкритий інбокс уже існує, він усе одно
  переписується — навіть до порожнього стану, — бо тіло має відображати ПОТОЧНИЙ
  стан БД, а не лишатися застарілим «N рядків» після того, як купу вже розгребли.
- **Насичення — стан, не гейт (#431).** Issue, що набрала `SATURATION_ALERT_ROWS` (12)
  ...
  Рахунок — це рядки, що **фактично долучились** (GitHub-коментар пройшов і запис у БД
  зроблено), а не заплановані планувальником: якщо `commentOnIssue` впав, ці рядки в
  лічильник не входять, тож невдалий коментар не робить issue насиченим ані в мітці, ані
  в рядку дайджесту (#431).
  Мітка звіряється (додати/зняти, ніколи не `PUT` усього набору) **після** всіх
  коментарів і записів у БД, по одному issue в своєму `try` — але лише на прогоні, що
  доходить до цієї фази запису. Прогін із порожнім батчем (`orphans.length === 0`) чи
  нульовими вердиктами (`covered === 0`) завершується раніше й мітки не чіпає: вона
  просто лишається такою, якою її залишив останній прогін, що таки дійшов до звірки —
  це коректно й нешкідливо, але це не «щопрогону».
...
- **Per-site адаптери** (`src/sites/`): кожен має стабільний `id` (= ім'я
  ...
  `morava-` → VibrantPour зі збереженням `Morava` в name; для
...
- **Popup керування кешем** (toolbar `action`, дозвіл `activeTab`):
  ...
  «оновити відкриту сторінку». Поки токен **не заданий**, popup показує «Not
  connected» і кнопку «Get a token» одним блоком (`#authBlock`) **одразу під
  шапкою**, а «Sync my check-ins» демотується в secondary, стає `disabled` і
  несе підпис `Add a token to sync your check-ins.` — без токена синк не
  опитується взагалі, тож збережений з минулого разу `outcome: 'error'` не може
  перебити цей підпис. Після збереження токена блок ховається, а посилання
  «Read the setup guide →» **лишається видимим** — воно живе в підвалі перед
  «Clear all cache» і переїжджає в `#authBlock` лише коли токена немає
  (#519, #522).
- **Popup — ієрархія подання:** порядок у DOM = порядок за важливістю **для
  поточного стану**. З токеном: «Sync my check-ins» (primary, залитий
  акцентом) → «Refresh this page» (secondary, контур) → згорнутий список
  «Supported shops» → підвал із посиланням на гайд і «Clear all cache»
  (ghost, під лінією-роздільником). Без токена першим після шапки йде
  `#authBlock` («Not connected» + «Get a token» у primary + посилання на
  гайд), а «Sync my check-ins» опускається в secondary й `disabled`.
  Список групує магазини за країною, з якої
  відправляються замовлення; кожен site adapter представлений рівно один раз, а
  натискання відкриває магазин в окремому browser window. Для магазину з кількома
  мовними storefront-адресами popup обирає одну за першою підтримуваною мовою браузера
  (`uk`/`pl`/`en`, fallback `en`); зокрема Beershop використовує `.pl` для польської
  та `.eu` для української/англійської.
  Первинна дія йде **першою**, тож у звичайному стані перший `Tab` потрапляє на
  неї, а не на деструктивну. Під час синхронізації вона лишається в tab-order і
  перетворюється на «Stop», тому навіть коли `refresh` disabled, «Clear all cache»
  не стає першою доступною дією. Без токена цей інваріант тримається вже з
  іншої причини: «Sync my check-ins» тут `disabled` і тому **випадає** з
  tab-order, а не «Clear all cache» відсуває сама себе — від першої позиції її
  утримує `#authBlock`, що лідирує з увімкненою «Get a token». Стан `disabled` має власні токени (заливка +
  приглушений текст), а не `opacity`. Popup підхоплює тему через `color-scheme: light dark`; акцент
  для заливки — темніший бурштин (AA-контраст з білим текстом), бренд-відтінок
  `#d9822b` лишається для мітки й фокус-рингу.
...
- **Бейджі (#648).** Стан картки — це один тип `CardState` і один рендер `renderState`
  (`extension/src/content/badge.ts`); інших способів намалювати бейдж нема. Дванадцять
  класів:

  | Клас | Бейдж | Клік |
  |---|---|---|
  | в черзі | кільце, тьмяне | — |
  | працюємо (`/match` або дошук у польоті) | дуга, обертається | — |
  | не пиво (вердикт крамниці) | `✕`, тьмяне | — |
  | знайшли, не пив | зірка + глобальна оцінка | сторінка пива |
  | знайшли, не пив, рейтингу нема (<10 оцінок) | зірка | сторінка пива |
  | знайшли, пив, є твоя оцінка | галочка + **твоя** оцінка | сторінка пива |
  | знайшли, пив, оцінки не ставив | галочка + зірка з глобальною | сторінка пива |
  | знайшли, пив, жодної оцінки | галочка | сторінка пива |
  | непевний збіг | будь-яке «знайшли» + пунктирна рамка й `?` | сторінка пива |
  | не знайшли | лупа | пошук Untappd |
  | не встигли (ліміт дошуку **або** бюджет #279) | стрілка перезавантаження, тьмяна | — |
  | помилка | трикутник із `!` | — |

  **Провенанс числа несе форма:** число без зірки після галочки — твоя оцінка, число із
  зіркою — глобальна. **Колір носить лише сигнал** — галочка зелена, зірка золота, бо це
  єдині картки, між якими людина обирає; процес і тупики кольору не мають, і `✕` більше
  не червоний. Колір ніде не єдиний носій змісту: форма розрізняє класи сама, і кожен
  бейдж має `role="img"`, `aria-label`, який промовляє стан екранному читачеві, і
  нативний `title` з тим самим текстом для наведення миші. Пасивний бейдж може бути
  hover-ціллю; якщо адаптер крамниці має контроль у тому самому місці, він зобов'язаний
  розташувати бейдж так, щоб його не перекрити. Іконки —
  інлайновий SVG зі стилями в `style` (презентаційні атрибути програли б будь-якому
  стилю крамниці), обертання вимикається під `prefers-reduced-motion`.

  Непевність (`source: 'fuzzy'`, зокрема суперечливий bid із крамниці — §POST /match)
  показується **і на невипитому пиві**: до #648 fuzzy на невипите виглядало як exact.
  Для непевного збігу сервер не віддає особистої оцінки взагалі, тож підпис каже «Схоже,
  ти це пив», а не «оцінки не ставив» — відсутність числа там нічого не доводить.

  Картка без бейджа означає рівно одне: оверлей на цій сторінці не працює
  (`isNonBeerPage` — ціла категорія не-пива).

  **Картка з бейджем помилки вважається обробленою** (`markSeen`), як і будь-який інший
  кінцевий стан. Це не косметика: `renderState` завжди замінює бейдж, тож сам бейдж
  помилки є мутацією DOM, яку бачить re-render observer — без `markSeen` він перечитував
  би картку як необроблену й дзвонив `/match` кожні 250 мс проти сервера, який щойно
  впав. Раніше циклу не було лише тому, що `setNonBeer` коротив на незміненому бейджі.
  Повторна спроба лишається доступною двома живими шляхами: перемальовка сітки самою
  крамницею дає нові вузли без мітки, а «Refresh this page» у popup кличе `resetCard`.
