...
| 15 | `job_state(key, value)` — дрібний крос-рестарт стан джоб (наразі `daily_status_last_sent`) |
...
**Untappd circuit breaker (in-memory).** Усі VPS-originated Untappd-звернення
гейтяться спільним in-memory circuit breaker: `refreshOntap` inline enrich,
...
breaker відкривається на 6 год (потім half-open probe); Untappd-частини jobs у цей
час пропускаються. Ontap scraping продовжується, але без inline enrich. Алерти
адміну лише на переходах: trip (`closed→open`) і recovery (`open→closed`). Стан
скидається на рестарті. `CookieExpiredError` у `refreshAllUntappd` лишається
окремим session/cookie шляхом і сам по собі не є IP-ban сигналом. Browser/extension
relay не гейтиться цим breaker-ом: блок у браузері користувача не впливає на VPS
cooldown.
