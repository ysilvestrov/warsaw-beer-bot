...
після зрізу провідної пивоварні та кожну `COLLAB_SEP`-сторону після такого ж зрізу,
щоб слабкі однотокенні сторони (`Lièvre / Slake` → `Lièvre`) не губились.
Пошуковий запит enrich'у будується collab-aware `stripBreweryNoise` (стрипить
