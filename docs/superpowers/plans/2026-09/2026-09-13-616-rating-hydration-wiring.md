...
| # | Знахідка | Рішення |
|---|---|---|
| M1 | один нерозбірний запис або запис чужого bid валить усю пачку з 1000 bid; черга має сталий порядок, тож кожен наступний запуск бере ті самі bid — гідратор тихо зупиняється. Проба: 33 пачки, 0 розбіжних/нерозбірних записів, 47 `null` | користувач обрав виправити: мапа `hydrateByBid` — `HydratedBeer \| null`; явний `null` = «Algolia не знає» (бекоф), відсутній ключ = «доказу немає» (пропуск без записів, лічильник `skipped`, `warn` у лозі); відповідь без `results`/з іншою кількістю — як і раніше транзієнт. `bid-identity` читає обидва випадки як «немає запису» |
...
## AI-рев'ю PR #625 (2026-09-13)

Перший прогін впав на інфраструктурі (OpenAI 429, кредити вичерпано); після поповнення — 3 знахідки,
усі перевірені по коду й прийняті.

| # | Знахідка | Рішення |
|---|---|---|
| P1 | підпис «Global Rating (3.72)» без читабельного `data-rating` читався як «N/A»: `refreshAllUntappd` стирав рейтинг у `NULL` і ставив штамп | `global_rating_shown` лише за числом у `data-rating` або явним підписом «(N/A)»; інакше — як блоку немає |
| P2 | штамп профілю не скидав `rating_refresh_*`: новий bid, якого Algolia ще не індексувала, після вичерпаного бекофу назавжди випадав із гідратора | гілка з блоком у `recordProfileBeer` скидає бекоф; без блоку — не чіпає |
| P2 | `listRatingHydrationCandidates` з від'ємним `limit` віддавав майже всю чергу (`slice(0, -1)`) | `slice(0, Math.max(0, limit))` (джоба вже затискала, експортована функція — ні) |
| P2 (2-й прохід) | «читабельність» `data-rating` перевірялась `parseFloat`, тож `"3.72soon"` рахувався числом | сувора перевірка `^\s*\d+(?:\.\d+)?\s*$`; тест доповнено цим значенням |
