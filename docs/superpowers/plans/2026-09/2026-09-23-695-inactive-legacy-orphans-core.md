...
**Interfaces:** Consumes Task 1 active readers/writers. Produces `previewLegacyOrphanDisposition(db, input): LegacyDispositionPreview`, `applyLegacyOrphanDisposition(db, input, expected?: LegacyDispositionPreview): {episodeId: number; kind: 'applied' | 'noop'}`, `previewLegacyOrphanReopen(db, input): LegacyReopenPreview`, `applyLegacyOrphanReopen(db, input, expected?: LegacyReopenPreview): {episodeId: number; kind: 'reopened' | 'noop'}`. `LegacyDispositionInput` has `beerId`, `issueNumber`, `cardBrewery`, `cardName`, `cardAbv`, `reason`, `evidenceUrl`, `operator`, `at`. `LegacyReopenInput` has `episodeId`, `reason`, `evidenceUrl`, `operator`, `at`. Previews include all row/failure fields used by the apply comparison and exact alias/active-key collision state.
...
- [ ] **Step 3: Implement strict parser and runner using `scripts/repair-legacy-card.ts` conventions.** Parse flags **before** opening production DB or loading config. Parse `--card-abv absent` as `null`, otherwise finite decimal `0..100`; parse IDs as positive safe integers. No network call is needed. Check `MAX(schema_version) >= 35` for apply; dry run can print `readyToApply: false` but must not write. For a new action, obtain a fresh preview, print it, then call the domain writer with that preview. For `--apply` when the target already has the persisted active episode (or a closed episode for reopen), call the domain writer **without** a preview so its exact-audit no-op/conflict branch decides; print that result. Never make the CLI itself declare a no-op. Use `loadOperatorEnv`, `loadEnv`, `openDb`, and a `require.main === module` guard like `repair-legacy-card.ts`.
