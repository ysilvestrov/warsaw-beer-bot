...
Виконання: задачі 1–3 інлайн (`9d5e1e1`, `8aa5416`, `565226d`), задача 4 — імплементер (`4a28a16`).
Задача 2 — код написано до тестів (червоного кроку не було), покриття доведено 19/19 мутаціями.
Рев'ю (свіжий рецензент, увесь diff від `origin/main`): Critical — 0, Important — 3, Minor — 6.
Перевірено по коду й прод-БД; усе, крім I1 (обв'язка), виправлено окремим комітом.

| # | Знахідка | Рішення |
|---|---|---|
| I1 | `refreshTapRatings` лишається підключеним: `beer-page.ts` пише `0` для «N/A», `recordRatingNotFound` інкрементує `rating_refresh_count` з новим змістом | Передумова обв'язки: заміна крон-слота й видалення старої джоби — у тому ж PR/деплої, що й ядро; записано в спеці («Стадії») |
| I2 | v31 міняє зміст `rating_refresh_*`, не скидаючи старі значення | v31 скидає їх. Прод: 6 рядків з `rating_refresh_at`, `MAX(count) = 0` — вічних бекофів немає, але заявка таблиці була хибна |
| I3 | «bid невідомий» вирішувався за `bid` усередині відповіді, не за позицією; 200 без `results` давав бекоф усій пачці | `rawHydrate` зіставляє за позицією; немає `results` / інша кількість / чужий bid на позиції → помилка (транзієнт для джоби, `hydrate-failed` для `bid-identity`, який і так ловить виняток) |
| M1 | бамп лише через рейтинг покривався тестом; style/ABV-only — ні | тест «filling only an empty style or only an empty ABV» |
| M2 | класифікація помилок не тестувалась (5xx як блок, 429 як транзієнт вижили) | тест `HttpError(500)` → транзієнт, `HttpError(429)` → блок |
| M3 | `<` → `<=` на межі 30 днів вижила; зняття `db.transaction` вижило | тест межі додано; транзакція лишається заради одного fsync на пачку, тестом не доводиться (свідомо) |
| M4 | невідомий bid після вичерпаного бекофу не звіряється ніколи — всі 6 таких bid на проді з рейтингом 0 | нулі переводить у `NULL` міграція v31 (правило п. 1); термінальний бекоф записано в «Поза межами» |
| M5 | «штампи рівномірні» хибне — повторна звірка йде піками | формулювання в спеці виправлено; свідомо лишається |
| M6 | округлення «рівно посередині» може розійтися з Algolia на 0,01 | записано в «Поза межами» як ризик для обв'язки `refreshAllUntappd` |
| Nit | `limit: -1` → `slice(0, -1)` ширше за пачку | `Math.max(0, …)` + тест |

Dry-run v31 на байтовій копії прод-БД: нулів 327 → 0, злінкованих без рейтингу 2272 → 2599,
додатних 30 228 → 30 228 (сума незмінна), `rating_refresh_at` 6 → 0, 10 мс.
