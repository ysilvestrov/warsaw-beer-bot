...
- [ ] **Після мерджу задеплоїти і перевірити на живому проді**, що шлях справді ожив:
  ...
  - наступний прогін `refreshOntap` має дати нормальні по-пабові лічильники — `ontap index
    parsed` з правдоподібним `n`, без масового `ontap pub refresh failed` — а не нулі чи суцільний
    провал: це шлях із найбільшим трафіком і найменшим прямим мережевим покриттям.
