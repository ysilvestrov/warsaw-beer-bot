...
- **Known limitation (flagged for review):** delikatesy soft drinks are caught by the whole-page `isNonBeerPage` gate, not per-product — so if a real beer were ever listed under `/delikatesy`, it would be skipped. The user's model treats `/delikatesy` as all-non-beer (no exception given), unlike `/szklanki-i-akcesoria` (handled per-product because it holds the MAGIC ROAD beer).
