...
Verified end-to-end (fixture `tests/fixtures/untappd-search/murphys.html`): input `Murphy's Brewery` / `Murphy's Irish Stout` matches `Heineken Ireland — Murphy's Irish Stout` (bid 5932) and correctly **rejects** the four other Murphy variants on the same page: `Mike Murphy's…` and `Murphy's Law…` fail the exact name-key stage (different keys), while `Murphys Dry…` and `Murphy And Son's…` fail the brand-in-name gate (the apostrophe means the input brewery normalizes to the two tokens `murphy s`, which is not a contiguous run in `murphys …` / `murphy and son s …`). FP control: `Pinta` / `Atak Chmielu` vs an unrelated brewery with the same beer name (brand not in the name) → `not_found`.
