...
2. **onemorebeer DOM (з рендер-фікстури).** Картка ← `.one-product-list-view__tile`;
   ...
   (з опційним стрипом префікса-пивоварні); **ABV вшито в тайтл** як `15,0°` →
   regex `/(\d+[,.]?\d*)\s*°/`, кома→крапка (окремої ноди `Moc` на тайлі нема;
   `abv` опційний). Рання assumption про ноду `Producent:`/`Moc (%)` стосувалася
   фільтр-сайдбара, не тайла — скоригована.
...
- `onemorebeer.ts` (Nuxt SPA): картка ← `.one-product-list-view__tile`; brewery ←
  ...
  name ← `a.product__title h2` (стрип префікса-пивоварні); abv ← regex `°` з тайтла
  (опційно). **Визначає `waitForGrid`** (чекає появи `.one-product-list-view__tile`)
  **і `reRenderContainerSelector`** (контейнер тайлів — для re-render observer §3.4).
