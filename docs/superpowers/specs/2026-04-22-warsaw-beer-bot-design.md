...
- **Configuration:** Litestream runs as a background service on the VPS, monitoring the SQLite file and streaming WAL (Write-Ahead Log) changes to R2.
- **Secrets:** R2 credentials should be provided via environment variables (.env) or a secure litestream.yml file, NEVER hardcoded.
...
- **i18n (uk + pl + en)**: усі user-facing рядки йдуть через `ctx.t(...)`.
  Підтримувані локалі — `uk` / `pl` / `en` (всі три повні; `be` — Phase 3).
  Дизайн — `docs/superpowers/specs/2026-04-27-i18n-design.md`. Архітектура —
  ...
  `en`. Команда `/lang` дозволяє явно перевизначити автодетект — інлайн-
  клавіатура з трьома варіантами; вибір persist у тій самій колонці.
  Артефакти review-циклу для PL/EN — `docs/i18n/{pl,en}-translations-review.md`
  (Status: final).
