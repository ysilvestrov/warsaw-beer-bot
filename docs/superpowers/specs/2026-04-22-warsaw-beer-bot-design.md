...
bot /newbeers → для кожного пабу: latest_snapshot → tapsForSnapshot
             ...
             → ctx.replyWithHTML
```
Один пивний рядок ≈ дві строки: жирне ім'я + ⭐ рейтинг, нижче список пабів.
...
- `/route [N]` — маршрут, що покриває ≥ N непитих пив; default N береться з
  ...
  N×N OSRM walking-distance матрицю з haversine-fallback.
...
- **Telegraf `handlerTimeout` = 90 с**. Для будь-якого handler'а, що чекає на
  довгу зовнішню роботу (refresh = ~3 хв на 43 паби), треба fire-and-forget +
  `editMessageText` через захоплені `ctx.telegram` + `chatId` + `messageId`
  (PR #14). Інакше Telegraf кидає `TimeoutError` у `bot.catch` через 90 c, а
  користувач у Telegram бачить лише початковий «⏳ Оновлюю…».
