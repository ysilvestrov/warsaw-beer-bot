...
- `/route [N]` — маршрут, що покриває ≥ N непитих пив; default N береться з
  `user_filters.default_route_n` або `env.DEFAULT_ROUTE_N` (=5). Передраховує
  N×N OSRM walking-distance матрицю з haversine-fallback. **Fire-and-forget**:
  handler миттєво відповідає `⏳ Будую маршрут…` і відчіплює роботу; матриця
  для ~30 пабів — це ~435 послідовних HTTPS-викликів до публічного OSRM
  (хвилини), тож handler-timeout 90с інакше вбивав би його. Live-progress через
  `editMessageText` з тим же троттлом 2с, що і `/refresh`.
...
- **Telegraf `handlerTimeout` = 90 с**. Для будь-якого handler'а, що чекає на
  ...
  - Майбутня оптимізація `/route`: переїхати на OSRM `/table` API (одна
    відповідь на матрицю замість N²/2 викликів) і кешувати дистанції в БД
    (paby не рухаються).
