...
**Правило для всіх даних:** жоден шлях не пише `rating_global = 0`. Наявні 327 нулів переводить у
`NULL` міграція v31 (п. 2) — за цим самим правилом 0 означає «рейтингу немає», тож перезапис не
втрачає інформації. (Первісно спека доручала їх гідратору; рев'ю ядра показало, що всі 6 bid, яких
Algolia не знає, — саме нульові рядки, і гідратор їх ніколи не звірив би.)
...
Algolia не знає** (їх читає й пише лише стара джоба, яка видаляється). Старі значення цього не
стверджують (HTML-джоба писала «на сторінці немає рейтингу» і транзієнти), тож v31 їх **скидає**.

v31 також виконує `UPDATE beers SET rating_global = NULL WHERE rating_global = 0` (п. 1).
Dry-run на байтовій копії прод-БД 2026-09-13: нулів 327 → 0, злінкованих без рейтингу 2272 → 2599,
додатних рейтингів 30 228 → 30 228 (сума незмінна), `rating_refresh_at` 6 → 0, 10 мс.
...
- у `hits` для bid явний `null` (Algolia цього bid не знає) → лише `rating_refresh_at = nowIso`,
  `rating_refresh_count + 1`; рейтинг і штамп не чіпаються;
- ключа bid у `hits` немає (запис не розібрався або на його позиції інший bid — доказу немає) → нічого не
  пишеться, лічильник `skipped` (рев'ю гілки, M1: один поганий запис не зупиняє пачку, а сталий порядок
  черги не повторює ту саму невдачу щозапуску);
...
- `limit = 1000` bid на запуск → **один** виклик `search.hydrateByBid(bids)` (межа Algolia — 1000
  ...
  у перших трьох запусках. Штампи **не** розтікаються рівномірно: бекфіл ставить ~33 тис. штампів за
  ~4–5 днів, тож через 30 днів той самий блок знову стає до черги разом (~4 зайняті дні, ~26 порожніх).
  При 1000 bid на запуск це нешкідливо і лишається свідомо.
- Помилки:
  ...
  - інша помилка (5xx, мережа, парсинг) → лог `warn`, нічого не пишеться, наступний запуск повторить;
    сюди ж — відповідь 200 без `results` або з іншою кількістю результатів: `rawHydrate` зіставляє
    `results[i]` з `bids[i]` і кидає помилку. `null` на позиції → явний `null` у мапі (bid невідомий);
    запис, що не розібрався або належить іншому bid, → bid без ключа (пропуск), решта пачки звіряється;
...
| Сторінка | Запис |
|---|---|
| блок з числом | `rating_global = число`, `rating_checked_at = now`, бекоф скидається |
| блок з «N/A» (`data-rating="0"` або підпис «(N/A)») | `rating_global = NULL`, `rating_checked_at = now`, бекоф скидається |
| блоку немає, або підпис без читабельного `data-rating` | рейтинг, штамп і бекоф не чіпаються |

(AI-рев'ю PR #625: підпис «Global Rating (3.72)» без читабельного значення раніше читався як «N/A» і стирав
рейтинг; вичерпаний бекоф після штампу профілю назавжди виключав рядок із гідратора.)
...
| `rating_checked_at = T` (`refreshAllUntappd`) | Блок `Global Rating` на сторінці `/beers` у момент T знайдено і він читається: число в `data-rating` (0 = менш ніж 10 оцінок) або явний підпис «(N/A)». Відсутній блок чи підпис без читабельного значення штампа не дає. |
| бекоф `rating_refresh_*` | Algolia на запит 200 повернула `null` саме на позиції цього bid, і кількість результатів дорівнює кількості запитаних (`rawHydrate` перевіряє) — у мапі це явний `null`, а не відсутній ключ. Недовірений запис, транзієнт і блок бекофу не пишуть; старі значення HTML-джоби скинуто в v31. |
| `rating_global` ніколи не `0` | Правило межі парсерів (п. 1): усі п'ять входів Untappd-рейтингу через один хелпер; наявні нулі — міграція v31 (dry-run на копії прод-БД); ontap `u_rating` нулів не має (виміряно). До деплою обв'язки `beer-page.ts` старої джоби ще пише 0 — тому ядро без обв'язки не деплоїться (див. «Стадії»). |
...
**Ядро й обв'язка деплояться разом (один PR).** Поки `refreshTapRatings` підключений, його
`beer-page.ts` пише `0` для пива з «N/A» на крані, а `recordRatingNotFound` інкрементує
`rating_refresh_count`, який відтепер означає «Algolia не знає bid». Одночасна робота двох джоб
ламає і правило «ніколи не 0», і зміст бекофу.

## Поза межами

- Рейтинги сиріт (`untappd_id IS NULL`) — у них немає bid.
- Повторна звірка bid, якого Algolia не знає, після вичерпаного бекофу (0/72/168/728 год): рядок лишається
  з тим рейтингом, що мав (після v31 — `NULL` або справжній), і в «звірено за 30 днів» не входить. На
  2026-09-13 таких bid 47 по всьому каталогу (6 — серед рядків без рейтингу; проба рецензента гілки на
  всіх 32 759 злінкованих bid: 33 запити, жодного розбіжного чи нерозбірного запису).
- Округлення «рівно посередині» (`x.xx5`) може розійтися з округленням Algolia на 0,01: сторінка й
  Algolia збіглися 25/25, але пари на межі у вибірці не було. Якщо обв'язка додасть у
  `refreshAllUntappd` порівняння «змінилось», таке пиво може бампати кеш на кожному проході.
