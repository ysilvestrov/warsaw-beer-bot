...
- Помилки:
  ...
  - інша помилка (5xx, мережа, парсинг) → лог `warn`, нічого не пишеться, наступний запуск повторить;
    сюди ж — відповідь 200, де позиції не сходяться з запитом: `rawHydrate` зіставляє `results[i]` з
    `bids[i]` і кидає помилку, якщо `results` немає, їх інша кількість або на позиції запис іншого
    bid (`null` на позиції = bid невідомий);
...
| бекоф `rating_refresh_*` | Algolia на запит 200 повернула `null` саме на позиції цього bid, і кількість результатів дорівнює кількості запитаних (`rawHydrate` перевіряє). Транзієнт і блок бекофу не пишуть; старі значення HTML-джоби скинуто в v31. |
