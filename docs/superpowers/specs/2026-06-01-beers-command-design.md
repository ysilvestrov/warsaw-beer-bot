...
- `{status}`: `matched`, якщо `beer_id != null`, інакше `orphan`
  — **завжди англійською в усіх локалях** (фіксовані рядки, не через `t()`)
...
Статуси `matched`/`orphan` — **не** в i18n (фіксовані англійські рядки в білдері).
