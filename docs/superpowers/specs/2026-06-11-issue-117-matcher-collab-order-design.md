...
nameKeys(rawName, brewery): Set<string>
  для кожного side у rawName.split(COLLAB_SEP):        // '/', ' x ', ' & '
    ...
    key  = norm.split(' ').filter(непорожні).sort().join(' ')    // ORDER-INSENSITIVE
    додати key, якщо непорожній
```
...
- `matchPrepared` **exact-стадія**: замість `c.nameNorm === nn` — перетин `nameKeys`.
  Брюері-gate і vintage/ABV-логіка лишаються; fuzzy-фолбек (≥ 0.75) із divergence-guard
  лишається без змін.
- `lookupBeer` **Stage 2**: перед fast-fuzzy спробувати перетин ключів на
  brewery-gated результатах; якщо перетину нема — поточний fuzzy ≥ 0.85 як фолбек.
...
- **`matcher.test.ts`:** `nameKeys`/`stripLeadingBrewery` — 5 позитивних кейсів + FP-проби
  (`Hazy Mango`↛`Hazy`, різні пива, style-only-назва).
