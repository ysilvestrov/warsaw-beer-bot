...
describe('v32 beer_aliases (#614)', () => {
  it('creates beer_aliases keyed by the exact card text of brewery and name and the card ABV', () => {
    const db = openDb(':memory:');
    migrate(db);
    const cols = (db.prepare('PRAGMA table_info(beer_aliases)').all() as { name: string }[])
      .map((c) => c.name);
    expect(cols).toEqual([
      'id', 'beer_id', 'brewery', 'name', 'brewery_text', 'name_text', 'abv_key', 'created_at',
    ]);
    db.prepare(`INSERT INTO beers (id, untappd_id, name, brewery, normalized_name, normalized_brewery)
                VALUES (8, 5940, 'Leffe Blonde / Blond', 'Abbaye de Leffe', 'leffe blonde blond', 'leffe'),
                       (10, 2948556, 'Leffe Blonde / Blond 0,0%', 'Abbaye de Leffe', 'leffe blonde blond 0 0', 'leffe')`).run();
    const insert = db.prepare(
      `INSERT INTO beer_aliases (beer_id, brewery, name, brewery_text, name_text, abv_key, created_at)
       VALUES (?, ?, ?, ?, ?, ?, '2026-09-14T07:13:20Z')`,
    );
    insert.run(8, 'LEFFE', 'BLONDE', 'leffe', 'blonde', '6.6');
    // Той самий текст з іншим ABV — інший ключ: 0%- і алкогольна версія живуть поруч.
    insert.run(10, 'LEFFE', 'BLONDE', 'leffe', 'blonde', '0');
    // Той самий текст і ABV удруге відмовляються.
    expect(() => insert.run(10, 'LEFFE', 'BLONDE', 'leffe', 'blonde', '6.6')).toThrow(/UNIQUE constraint failed/);
  });
