...
describe('#427 upstream identity evidence', () => {
  ...
  test('#465 identity alias: bare brewery brand does not admit an unrelated beer when name is empty', async () => {
    const search = fakeSearch(() => [
      {
        bid: 999999,
        beer_name: 'Random Beer',
        brewery_name: 'Other Brewery',
        style: 'IPA',
        abv: 5,
        global_rating: 3.5,
        alias_alt: ['Magic Road'],
      },
    ]);

    const out = await lookupBeer({ brewery: 'Magic Road', name: '', abv: 5, search });
...
describe('#405 Sub-cohort A3: swapped brewery and beer name', () => {
  ...
  test('not_found: swapped resolution refuses candidate with null ABV (strict corroboration required)', async () => {
    const search = fakeSearch(() => [
      {
        bid: 6757171,
        beer_name: 'PŁYNNE ZŁOTO',
  ...
  test('not_found: swapped resolution refuses candidate beer with extra unswapped tokens', async () => {
    const search = fakeSearch(() => [
      {
        bid: 6757172,
        beer_name: 'PŁYNNE ZŁOTO IPA',
        brewery_name: 'Browar Dziki Wschód',
        style: 'IPA',
        abv: 7.5,
        global_rating: 3.7,
  ...
  test('matched: swapped resolution deduplicates candidate rows with identical bid', async () => {
    const search = fakeSearch(() => [
      {
        bid: 6757171,
        beer_name: 'PŁYNNE ZŁOTO',
