...
export const VerdictSchema = z.object({
  ...
  // an existing open issue OR a new_issues entry; not_on_untappd/unidentifiable use neither.
...
export function buildTriagePrompt(input: TriageInput): string {
  ...
  return [
    ...
    '   garbled (typos in the shop\'s data, e.g. "BRAURIE KEESMANN", "NAPOMUCEN"), our',
    '   adapter read it correctly — that is NOT parser_bug; keep walking the tree.',
    ...
    '   Brewery alias gap (rebrand, contract brewer, Cyrillic/transliteration), name',
    '   divergence (translation, word order, minor typo), or query noise that only needs',
    '   normalising before search (bracketed adjunct lists, ABV/spec strings, collab',
    '   parentheticals). Candidates that nearly match are a strong hint.',
    '',
    ...
    'not_on_untappd/unidentifiable, or matcher_bug with issue_number: null.',
    ...
    '- not_on_untappd / unidentifiable verdicts must have issue_number: null and new_issue_key: null.',
