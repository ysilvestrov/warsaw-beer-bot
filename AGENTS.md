...
- **No weak assertions**: Prefer `.toBe()` or `.toEqual()` with exact expected values over `.toBeGreaterThanOrEqual()`, `.toBeTruthy()`, or `.toBeDefined()`. A test that passes for both the right and wrong value is worthless. **An exact assertion pins the test's own fact, not current state that the next code change will move**: when replacing a weak assertion, first name what the test claims and assert that (a test of migration v20 asserts v20 is recorded — `WHERE version = 20` — not `MAX(version) = 34`). A global value that grows with the code (schema head, registry size, number of supported shops) is pinned by **exactly one** test, so a new change edits only that one. Why — #701/#708: `>= 20` was replaced with `toBe(34)`, a parallel branch adding migration v35 collided with that line only after both merged, and the fix deleted the assertion instead of correcting it.
...
- **`verify` is where the money is now, not `find`.** Measured across 12 draws on 2026-09-23: verify is 38–47% of a run with the incumbent find model and **94–96%** with a cheap one. The "≈18%" this repo quoted came from PRs carrying one or two findings; on a PR with seven, the judge costs what the hunter does. Also: quote a candidate's **per-draw scores and the union together** — they are different numbers, and comparing one config's union against another's per-draw score is the 2026-09-22 mistake wearing a different hat.
- **cheap does not mean thoughtless.** `gpt-6-luna` spent MORE reasoning tokens than the incumbent (4.1–4.9k vs 2.6–3.1k) at 1/38 the find cost, and still under-covered. When a cheap model misses defects, name the blind spot — do not explain it with the price, because the next cheap model will not have the same one.
- **compare vendors by measured tokens, never by the price row.** A price per 1M tokens only becomes a cost once multiplied by the tokens *that vendor* counts for your prompt: on identical text Anthropic counted 1.65× OpenAI's tokens (2 850 vs 1 725 on the same 6 596-char file, 2026-09-23), which is enough to flip a ranking — `claude-opus-5-5` looks price-identical to `gpt-5.6-sol` and is ~26% dearer per review call. And weigh output as heavily as input: with a reasoning model on `find`, output was 60% of that stage's cost on PR #688, so the old "input dominates" note from #364 no longer holds.
- **probe each vendor's request shape and reasoning switch; never infer either from another vendor.** One free structured call answers both. Anthropic takes our exact body — `max_completion_tokens` plus strict `json_schema` — directly over its OpenAI-compatibility endpoint, so no code seam is needed to measure it; but that same endpoint accepts `reasoning_effort` and a native `thinking` block and silently ignores both. A candidate measured with reasoning off can be cleared by that test, never rejected by it.
...
The full protocol, the labelled corpus, and every pass's numbers are in `docs/ai-review-model-evaluation.md` (protocol first, then an append-only results log). A **verify** candidate is scored against `docs/ai-review-fabrication-corpus-2026-09-22.md` — 16 wrong claims a good judge rejected — because the recall probe cannot measure a judge.
